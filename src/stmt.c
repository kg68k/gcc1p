     ...
     int line;
{
  ...
  if (ninputs + noutputs > MAX_RECOG_OPERANDS)
    {
      error ("operand は最大%d個です", MAX_RECOG_OPERANDS);
      return;
    }
     ...
     int dont_jump_in;
{
  ...
  if (warn_unused)
	...
	warning_with_decl (decl, "`%s'が未使用です");
     ...
     tree type;
{
  ...
  for (chain = TYPE_VALUES (type);
	...
	warning ("switchで列挙値`%s'の処理がありません",
  ...
  for (n = case_stack->data.case_stmt.case_list; n; n = n->right)
	...
	;
      ...
      if (!chain)
	warning ("case値`%d'はenum`%s'に存在していません",
...
#endif /* 0 */
#ifdef __human68k__
	  /* entry_parm is not 0, so param arrived register. if vararg then
	     copy to stack. But, if TREE_CHAIN (parm) is continue 2 more,
	     it is not need. */
	  if (vararg)
	    {
	      int i;
	      tree next = TREE_CHAIN (parm);
	      if (next == 0 && REGNO (entry_parm) == 0)
		{
		  rtx stack = validize_mem (stack_parm);
		  current_function_pretend_args_size = 8;
		  i = 2;
		  while (--i >= 0 )
		    emit_move_insn (gen_rtx (MEM, SImode,
					     plus_constant (XEXP (stack, 0),
							    i * GET_MODE_SIZE (SImode))),
				    gen_rtx (REG, SImode, REGNO (entry_parm) + i));
		  entry_parm = stack_parm;
		  nregs = 0;
		}
	      else if ((next && TREE_CHAIN (next) == 0 && REGNO (entry_parm) == 0)
		       || (next == 0 && REGNO (entry_parm) == 1))
		{
		  rtx stack = validize_mem (stack_parm);
		  current_function_pretend_args_size = 8;
		  i = REGNO (entry_parm);
		  emit_move_insn (gen_rtx (MEM, SImode, XEXP (stack, 0)),
  ...
  for (decl = STMT_VARS (block); decl; decl = TREE_CHAIN (decl))
    {
	  ...
	  && regno_uninitialized (REGNO (DECL_RTL (decl))))
        if (!( REGNO (DECL_RTL (decl)) < 16 && inter_use_regs[ REGNO (DECL_RTL (decl))]))
	    warning_with_decl (decl,
			     "`%s'が初期化されずに使用されているようです");
      if (TREE_CODE (decl) == VAR_DECL
	  && DECL_RTL (decl) != 0
	  && GET_CODE (DECL_RTL (decl)) == REG
	  && regno_clobbered_at_setjmp (REGNO (DECL_RTL (decl))))
	warning_with_decl (decl,
			   "`%s'は`longjmp'で破壊される可能性があります");
    }
  ...
  if (warn_unused)
    for (decl = DECL_ARGUMENTS (current_function_decl);
	 decl; decl = TREE_CHAIN (decl))
      if (! TREE_USED (decl) && TREE_CODE (decl) == VAR_DECL)
	warning_with_decl (decl, "`%s'は未使用の引き数です");
#endif
