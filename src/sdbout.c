...
#ifdef __human68k__
#include "sdbinf.h"
#define	C_SX_REMOTE_EXT    80
#define	C_SX_REMOTE_STAT   81
#define	C_SX_RELOCATE_EXT  82
#define	C_SX_RELOCATE_STAT 83
#define SXVAL_LK
#endif
  ...
  if (local == 0)
    {
      ...
      sdbout_types (nreverse (get_permanent_types ()));
#else
      tree tem = (tree) get_permanent_types ();
      sdbout_types (tem);
      nreverse (tem);
      sdbout_tags (gettags ());
#endif
  ...
  switch (TREE_CODE (decl))
	  ...
	  && GET_CODE (XEXP (value, 0)) == SYMBOL_REF)
	{
	  if (TREE_PUBLIC (decl))
	  ...
	  else
	    {
		  ...
		  PUT_SDB_VAL (XEXP (value, 0));
#endif
		  PUT_SDB_SCL (C_SX_RELOCATE_STAT);
		}
	      else
		{
		  PUT_SDB_VAL (XEXP (value, 0));
	       ...
	       && GET_CODE (XEXP (XEXP (value, 0), 1)) == CONST_INT)
	{
	  ...
	  PUT_SDB_SCL (C_AUTO);
	       ...
	       && GET_CODE (XEXP (value, 0)) == PLUS
	       && GET_CODE (XEXP (XEXP (value, 0), 0)) == REG
	       && REGNO (XEXP (XEXP (value, 0), 0)) == 13
	       && GET_CODE (XEXP (XEXP (value, 0), 1)) == SYMBOL_REF)
	{
#ifdef SXVAL_LK
	  PUT_SDB_VAL (XEXP (XEXP (value, 0), 1));
#endif
	  if (TREE_PUBLIC (decl))
	    PUT_SDB_SCL (C_SX_REMOTE_EXT);
	  else
	    PUT_SDB_SCL (C_SX_REMOTE_STAT);
	}
#endif
      else
	{
