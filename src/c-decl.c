/* Process declarations and variables for C compiler.
   ...
   Copyright (C) 1991 06 09 FSHARP Mariko Modify Errmessage
     ...
     int functionbody;
{
  ...
  for (link = tags; link; link = TREE_CHAIN (link))
	...
	switch (TREE_CODE (type))
	  {
	  case RECORD_TYPE:
	    errmsg = "`struct %s'が不完全なままで終了しています";
	    break;
	  case UNION_TYPE:
	    errmsg = "`union %s'が不完全なままで終了しています";
	    break;
	  case ENUMERAL_TYPE:
	    errmsg = "`enum %s'が不完全なままで終了しています";
	    break;
	  }
     ...
     register tree newdecl, olddecl;
{
  ...
  if (TREE_CODE (olddecl) != TREE_CODE (newdecl))
    {
      error_with_decl (newdecl, "`%s'が別の宣言をされました");
      error_with_decl (olddecl, "`%s'の前宣言位置です");
    }
  else
    {
	...
	;
 	...
 	{
 	  if (!types_match)
 	    error_with_decl (newdecl, "`%s'がbuilt-in関数と衝突しました");
 	  else if (extra_warnings)
 	    warning_with_decl (newdecl, "built-in関数`%s'が再宣言されました");
 	}
      else if (!types_match)
	{
	  error_with_decl (newdecl, "`%s'と衝突しています");
	  ...
	  if (TREE_CODE (olddecl) == FUNCTION_DECL
	      ...
	      for (; t; t = TREE_CHAIN (t))
		{
		  ...
		  if (type == float_type_node
		    ...
		    {
		      error ("引数はデフォルト変換されます");
		      error ("空の引数宣言とは一致しません");
		      break;
		    }
		}
	    }
      ...
      else
	{
	  ...
	  if (errmsg)
		   ...
		   && TYPE_ARG_TYPES (TREE_TYPE (newdecl)) != 0)
	    {
	      ...
	      warning_with_decl (newdecl, "`%s'のプロトタイプ宣言が");
	  ...
	  if (pedantic
	    ...
	    warning_with_decl (newdecl, "`%s'の宣言が前のと異なっています");
	}
    }
#ifdef __human68k__
  {
    extern int flag_remote;
    if (flag_remote)
      TREE_REMOTE (newdecl) = TREE_REMOTE (olddecl);
  }
  if (TREE_CODE (olddecl) == TREE_CODE (newdecl)
      && (TREE_COMMON (newdecl) != TREE_COMMON (olddecl)
          || TREE_PCR_DATA (newdecl) != TREE_PCR_DATA (olddecl)
          || TREE_REMOTE (newdecl) != TREE_REMOTE (olddecl)
          || TREE_RELOCATE (newdecl) != TREE_RELOCATE (olddecl)
          || TREE_DOSCALL (newdecl) != TREE_DOSCALL (olddecl)))
    {
      error_with_decl (newdecl, "`%s'が別記憶クラス宣言されました");
      error_with_decl (olddecl, "`%s'の前宣言位置です");
      return 0;
    }
#endif
     ...
     tree x;
{
  ...
  if (name)
	...
	/* error_mark_node is 0 for a while during initialization!  */
	{
	  t = 0;
#ifdef __human68k__
          /* error_with_decl needs 3 params, why ??? */
	  error_with_decl (x, "`%s'が宣言の前に使われました",
      ...
      if (t != 0 && duplicate_decls (x, t))
	{
	  ...
	  if (!flag_traditional && TREE_PUBLIC (name)
	    ...
	    {
	      warning ("static`%s'が暗黙にexternに宣言されています",
		       IDENTIFIER_POINTER (name));
	  ...
	  && lookup_name (name) == 0)
	{
	  ...
	  while (type)
		...
		{
		  warning_with_decl (x, "`extern'が global ではありません");
		  break;
		}
      ...
      if (b == global_binding_level)
	{
	  ...
	  if (IDENTIFIER_IMPLICIT_DECL (name) != 0
	    ...
	    warning ("`%s'が前にintを返すと暗黙に宣言されています",
		     IDENTIFIER_POINTER (name));
#else
	  /* Error about mismatches against previous implicit decl.  */
	  if (IDENTIFIER_IMPLICIT_DECL (name) != 0)
	    if (flag_traditional || TREE_CODE (x) == FUNCTION_DECL)
	      {
	        /* implicit decl function type mismatch */
	        if (!(TREE_TYPE (TREE_TYPE (x)) == integer_type_node))
	    	  warning ("`%s'が前にintを返すと暗黙に宣言されています",
			   IDENTIFIER_POINTER (name));
	      }
	    else
	      warning ("`%s'は暗黙に関数として宣言されています",
	             IDENTIFIER_POINTER (name));
#endif
	  ...
	  if (TREE_PUBLIC (name)
	    ...
	    {
	      if (IDENTIFIER_IMPLICIT_DECL (name))
		warning ("static`%s'は暗黙にexternに宣言されています",
			 IDENTIFIER_POINTER (name));
	      else
		warning ("static`%s'はexternに宣言されています",
			 IDENTIFIER_POINTER (name));
	    }
	}
      else
	{
	  ...
	  if (oldlocal == 0
	      ...
	      && TREE_CODE (oldglobal) == FUNCTION_DECL)
	    {
	      ...
	      if (! comptypes (TREE_TYPE (x),
			       TREE_TYPE (IDENTIFIER_GLOBAL_VALUE (name))))
		warning_with_decl (x, "局所宣言`%s'が外部と一致しません");
	  ...
	  /* Maybe warn if shadowing something else.  */
		...
		warnstring = "`%s'の宣言は引数を隠します";
	      else if (oldlocal != 0)
		warnstring = "`%s'の宣言はlocal変数を隠します";
	      else if (IDENTIFIER_GLOBAL_VALUE (name) != 0)
		warnstring = "`%s'の宣言はglobal宣言を隠します";
     ...
     tree name;
{
  ...
  if (DECL_SOURCE_LINE (decl) != 0)
    {
      error_with_decl (decl, "ラベル`%s'が複数あります");
      return 0;
    }
  else
    {
...
#ifdef __human68k__
  {
    extern struct rtx_def *sx_ret_regs; /* This is rtx!! */
    tree decl;
    if (!flag_no_sx)
      {
        pushdecl (decl = build_decl (VAR_DECL, get_identifier ("_SXCALLPtr"),
	          ptr_type_node));
        sx_ret_regs = DECL_RTL (decl) = (struct rtx_def *) gen_rtx (34, 4, 8);
        TREE_USED (decl) = 1;
      }
  }
#endif
     ...
     tree declspecs;
{
  ...
  for (link = declspecs; link; link = TREE_CHAIN (link))
	...
	{
	  if (!warned)
	    warning ("意味のないkeyword,type nameが宣言にあります");
	  warned = 1;
	}
    }
     ...
     int initialized;
{
  ...
  if (initialized)
	...
	if (pedantic || list_length (declspecs) > 1)
	  {
	    error ("typedef `%s'が変数のように初期化されています",
     ...
     tree asmspec_tree;
{
  ...
  if (TREE_CODE (type) == ARRAY_TYPE
	...
	= complete_array_type (type, DECL_INITIAL (decl), do_default);
      ...
      if (failure == 1)
	error_with_decl (decl, "%sの初期値のサイズが決定できません");
      ...
      if (failure == 2)
	{
	  if (do_default)
	    {
	      if (! TREE_PUBLIC (decl))
		error_with_decl (decl, "配列`%s'の要素数がありません");
	    }
	  ...
	  && tree_int_cst_lt (TYPE_MAX_VALUE (TYPE_DOMAIN (type)),
			      integer_zero_node))
	error_with_decl (decl, "`%s'のサイズが０です");
  ...
  if (TREE_CODE (decl) == VAR_DECL)
      ...
      if (TREE_STATIC (decl) && DECL_SIZE (decl) == 0)
	{
	  ...
	  if (! (TREE_PUBLIC (decl) && DECL_INITIAL (decl) == 0))
	    error_with_decl (decl, "`%s'のサイズは不明です");
	}
	...
	error_with_decl (decl, "`%s'のサイズが定数ではありません");
    }
     ...
     int initialized;
{
	...
	for (i = (int) RID_FIRST_MODIFIER; i < (int) RID_MAX; i++)
	  {
		...
		if (i == (int) RID_LONG && specbits & (1<<i))
		  {
		    if (pedantic)
		      warning ("`%s'が複数あります", IDENTIFIER_POINTER (id));
		    else if (longlong)
		      warning ("long long longは長すぎます");
		    else
		      longlong = 1;
		  }
      ...
      if (type)
	error ("`%s'が複数のデータタイプを持っています", name);
      else if (TREE_CODE (id) == IDENTIFIER_NODE)
	{
	  register tree t = lookup_name (id);
	  if (!t || TREE_CODE (t) != TYPE_DECL)
	    error ("`%s'はtypedefかbuilt in typeできません",
  ...
  if (specbits & ((1 << (int) RID_LONG) | (1 << (int) RID_SHORT)
	...
	error ("`%s'のlong, short, signed , unsignedは不正です", name);
      else if ((specbits & 1 << (int) RID_LONG)
	       && (specbits & 1 << (int) RID_SHORT))
	error ("`%s'にlongとshort双方指定されています", name);
      else if (((specbits & 1 << (int) RID_LONG)
		|| (specbits & 1 << (int) RID_SHORT))
	       && explicit_char)
	error ("char `%s'にlong, short指定されています", name);
      else if ((specbits & 1 << (int) RID_SIGNED)
	       && (specbits & 1 << (int) RID_UNSIGNED))
	error ("`%s'にsignedとunsigned双方指定されています", name);
      else
	{
	  if (!explicit_int && !explicit_char && pedantic)
	    warning ("`%s'のlong, short, signed, unsigned指定は不正です",
		     name);
...

  {
    ...
    {
      int c = 0;
      if (specbits & 1 << (int) RID_COMMON) c++;
      if (specbits & 1 << (int) RID_PCR)
        if (specbits & 1 << (int) RID_COMMON)
          ;
        else
         c++;
      if (specbits & 1 << (int) RID_RELOCATE) c++;
      if (specbits & 1 << (int) RID_REMOTE)
        if (specbits & 1 << (int) RID_RELOCATE)
          ;
        else
         c++;
      if (specbits & 1 << (int) RID_DOSCALL) c++;
      if (nclasses <= 1 && c == 1)
        nclasses = 1;
      else if (nclasses == 2)
        ;
	...
	if (specbits & 1 << (int) RID_AUTO)
	  error ("関数の記憶クラスが`auto'です");
	if (specbits & 1 << (int) RID_REGISTER)
	  error ("関数の記憶クラスが`register'です");
	if (specbits & 1 << (int) RID_TYPEDEF)
	  error ("関数の記憶クラスが`typedef'です");
#ifdef __human68k__
	if (specbits & 1 << (int) RID_COMMON)
	  error ("関数の記憶クラスが`common'です");
	if (specbits & 1 << (int) RID_REMOTE)
	  error ("関数の記憶クラスが`remote'です");
	if (specbits & 1 << (int) RID_RELOCATE)
	  error ("関数の記憶クラスが`relocate'です");
	if (specbits & 1 << (int) RID_PCR)
	  error ("関数の記憶クラスが`pcr'です");
#endif
	...
	if (decl_context == PARM && specbits & 1 << (int) RID_REGISTER)
	...
	else
	  {
	    error ((decl_context == FIELD
		    ? "構造体の要素 %s に記憶クラスが指定されています"
		    : (decl_context == PARM
		       ? "引数 %s に記憶クラスが指定されています"
		       : "typenameに記憶クラスが指定されています")),
		   name);
	    specbits &= ~ ((1 << (int) RID_TYPEDEF) | (1 << (int) RID_REGISTER)
	     ...
	     && ! funcdef_flag)
      warning ("`%s'を`extern'で初期化しようとしています", name);
    else if (current_binding_level == global_binding_level
	     && specbits & (1 << (int) RID_AUTO))
      error ("top-levelでの`auto'は不正です", name);
#ifdef __human68k__
    else if (current_binding_level != global_binding_level
	     && !(specbits & (1 << (int) RID_EXTERN))
	     && !(specbits & (1 << (int) RID_STATIC))
	     && ((specbits & (1 << (int) RID_COMMON))
	         || (specbits & (1 << (int) RID_RELOCATE))
	         || (specbits & (1 << (int) RID_PCR))))
      error ("auto変数の`common',`relocate',`pcr'は不正です", name);
#endif
  }
  ...
  while (declarator && TREE_CODE (declarator) != IDENTIFIER_NODE)
      ...
      if (TREE_CODE (declarator) == ARRAY_REF)
	{
	  ...
	  if (type == void_type_node)
	    {
	      error ("`%s'がvoidの配列です", name);
	      type = error_mark_node;
	    }

	  if (TREE_CODE (type) == FUNCTION_TYPE)
	    {
	      error ("`%s'が関数の配列です", name);
	      type = error_mark_node;
	    }
	  ...
	  if (size)
		...
		{
		  error ("配列`%s'のサイズが整数ではありません", name);
		  size = integer_one_node;
		}
	      if (pedantic && integer_zerop (size))
		warning ("ANSI Cではサイズ０の配列`%s'は使えません", name);
	      if (TREE_CODE (size) == INTEGER_CST)
		{
		  if (INT_CST_LT (size, integer_zero_node))
		    {
		      error ("配列`%s'のサイズが負です", name);
		      size = integer_one_node;
		    }
		...
		{
		  if (pedantic)
		    warning ("ANSI Cでは可変サイズ配列`%s'は使えません", name);
	  ...
	  if (type == error_mark_node)
	    continue;
          ...
          if (pedantic && (constp || volatilep))
            warning ("ANSIではconst,volatile関数は使えません");
#ifdef __human68k__
          if (constp && volatilep)
            error ("関数`%s' constでvolatileな関数は違法です", name);
#endif
	  /* Warn about some types functions can't return.  */

	  if (TREE_CODE (type) == FUNCTION_TYPE)
	    {
	      error ("関数`%s' 関数を返すことはできません", name);
	      type = integer_type_node;
	    }
	  if (TREE_CODE (type) == ARRAY_TYPE)
	    {
	      error ("関数`%s' 配列を返すことはできません", name);
	      type = integer_type_node;
	    }
...
#ifdef __human68k__
          if ((specbits & (1 << (int) RID_DOSCALL))
	      && TREE_CHAIN (TREE_OPERAND (declarator, 1)) == 0
	      && ! (funcdef_flag
	            && TREE_CODE (TREE_OPERAND (declarator, 0)) == IDENTIFIER_NODE))
            error ("関数`%s' はプロトタイプ必須です", name);
#endif
...
/*      layout_type (type);  */
...

  {
	...
	if (TREE_CODE (type) == ARRAY_TYPE)
	  ...
	  {
	    if (pedantic && (constp || volatilep))
	      warning ("ANSI Cではconstやvolatileである関数は違反です");
	    type = build_pointer_type (c_build_type_variant (type, constp, volatilep));
	    volatilep = constp = 0;
	  }

	if (initialized)
	  error ("引数`%s'が初期化されています", name);
	...
	if (current_binding_level != global_binding_level
	    ...
	    && pedantic)
	  warning ("関数`%s'の記憶クラスが不正です",
		   IDENTIFIER_POINTER (declarator));
	decl = build_decl (FUNCTION_DECL, declarator, type);
#ifdef __human68k__
	TREE_DOSCALL (decl) = (specbits & (1 << (int) RID_DOSCALL)) ? 1 : 0;
	TREE_INTERRUPT (decl) = (specbits & (1 << (int) RID_INTERRUPT)) ? 1 : 0;
        if (TREE_DOSCALL (decl) && TREE_INTERRUPT (decl))
          {
            error ("割り込み関数の属性宣言が不正です");
            TREE_DOSCALL (decl) = 0;
            TREE_INTERRUPT (decl) = 0;
          }
#endif
	TREE_EXTERNAL (decl) = 1;
	...
	if (inlinep)
	  {
	    ...
	    if (! strcmp (IDENTIFIER_POINTER (declarator), "main"))
	      warning ("関数`main'はinlineにできません");
	    else if (last && TREE_VALUE (last) != void_type_node)
	      warning ("可変長引数関数はinlineにできません");
#ifdef __human68k__
	    else if (TREE_DOSCALL (decl))
	      error ("SXCALL関数はinlineにできません");
	    else if (TREE_INTERRUPT (decl))
	      error ("割り込み関数はinlineにできません");
#else
	    else
#endif
	...
	if (inlinep)
	  warning_with_decl (decl, "変数`%s'がinline宣言です");
...
#ifdef __human68k__
	if (specbits & (1 << (int) RID_DOSCALL))
	  error ("変数`%s'をSXCALLクラス指定しています", name);
	if (specbits & (1 << (int) RID_INTERRUPT))
	  error ("変数`%s'を割り込み関数宣言しています", name);
	if (flag_no_sx && !flag_pic)
	  TREE_COMMON (decl) = 1;
	else
	  TREE_COMMON (decl)
	    =  (specbits & (1 << (int) RID_COMMON)) ? 1 : 0;
	TREE_REMOTE (decl)
	  =  (specbits & (1 << (int) RID_REMOTE)) ? 1 : 0;
	TREE_RELOCATE (decl)
	  =  (specbits & (1 << (int) RID_RELOCATE)) ? 1 : 0;
	TREE_PCR_DATA (decl)
	  =  (specbits & (1 << (int) RID_PCR)) ? 1 : 0;
#endif
     ...
     int funcdef_flag;
{
  ...
  if (warn_strict_prototypes && first_parm == 0 && !funcdef_flag)
    warning ("関数宣言がprototypeではありません");

  if (first_parm != 0
      && TREE_CODE (TREE_VALUE (first_parm)) == IDENTIFIER_NODE)
    {
      if (! funcdef_flag)
	warning ("タイプの指定のない引数の名前が関数宣言に現れました");
	  ...
	  which uses a more accurate criterion for what to warn about.  */
	      else
		{
		  ...
		  if (TYPE_SIZE (type) == 0)
		    {
		      if (DECL_NAME (parm) != 0)
			warning ("引数`%s'が不完全なtypeを指しています",
				 IDENTIFIER_POINTER (DECL_NAME (parm)));
		      else
			warning ("引数が不完全なtypeを指しています");
		    }
		}
#endif
  ...
  if (code == ENUMERAL_TYPE)
    {
      ...
      if (pedantic)
	warning ("ANSI Cではenumの前方参照はできません");
     ...
     tree name;
{
  ...
  if (ref && TREE_CODE (ref) == code)
    {
      if (TYPE_FIELDS (ref))
	error ((code == UNION_TYPE ? "union %sが再定義されました"
		: "struct %sが再定義されました"),
	       IDENTIFIER_POINTER (name));
     ...
     register tree t, fieldlist;
{
  ...
  if (in_parm_level_p ())
    warning ((TREE_CODE (t) == UNION_TYPE ? "unionが内部宣言です"
	      : "structureが内部宣言です"));

  old_momentary = suspend_momentary ();

  if (fieldlist == 0 && pedantic)
    warning ((TREE_CODE (t) == UNION_TYPE ? "unionのメンバがありません"
	      : "structureのメンバがありません"));
  ...
  for (x = fieldlist; x; x = TREE_CHAIN (x))
	...
	{
	  error_with_decl (x, "bit-field `%s'の幅指定が整数ではありません");
	  DECL_INITIAL (x) = NULL;
	}
	...
	{
	  error_with_decl (x, "bit-field `%s'に不当なtypeがあります");
	  DECL_INITIAL (x) = NULL;
	}
      if (DECL_INITIAL (x) && pedantic
	  && TREE_TYPE (x) != integer_type_node
	  && TREE_TYPE (x) != unsigned_type_node)
	warning_with_decl (x, "bit-field `%s'はANSI Cでは不当です");
      ...
      if (DECL_INITIAL (x))
	{
	  ...
	  if (width < 0)
	    {
	      DECL_INITIAL (x) = NULL;
	      warning_with_decl (x, "bit-field `%s'の幅は負にできません");
	    }
	  else if (width == 0 && DECL_NAME (x) != 0)
	    {
	      error_with_decl (x, "bit-field `%s'のサイズが０です");
	      DECL_INITIAL (x) = NULL;
	    }
	  else if (width > TYPE_PRECISION (TREE_TYPE (x)))
	    {
	      DECL_INITIAL (x) = NULL;
	      warning_with_decl (x, "`%s'の幅が不当です");
	    }
	}
  ...
  for (x = fieldlist; x && TREE_CHAIN (x);)
	...
	if (DECL_NAME (y) == DECL_NAME (TREE_CHAIN (x)))
	  {
	    error_with_decl (TREE_CHAIN (x), "メンバ`%s'が２重です");
	    TREE_CHAIN (x) = TREE_CHAIN (TREE_CHAIN (x));
	  }
	else x = TREE_CHAIN (x);
      }
  ...
  if (TYPE_VALUES (enumtype) != 0)
    {
      ...
      error ("`enum %s'が再宣言されています", IDENTIFIER_POINTER (name));
     ...
     tree declarator, declspecs;
{
  ...
  if (TREE_INTERRUPT (current_function_decl))
    {
...
store_parm_decls ()
{
  ...
  if (specparms != 0 && TREE_CODE (specparms) != TREE_LIST)
      ...
      for (parm = specparms; parm; parm = next)
	{
	  ...
	  if (DECL_NAME (parm) == 0)
	    error_with_decl (parm, "引数がありません");
	  else if (TREE_TYPE (parm) == void_type_node)
	    error_with_decl (parm, "引数 `%s'はvoidと宣言されています");
      ...
      for (parm = specparms; parm; parm = TREE_CHAIN (parm))
	{
	  ...
	  if (TREE_VALUE (parm) == 0)
	    {
	      error_with_decl (fndecl, "引数の名前がありません");
	      TREE_PURPOSE (parm) = 0;
	      continue;
	    }
	  ...
	  if (found && DECL_CONTEXT (found) != 0)
	    {
	      error_with_decl (found, "引数`%s'が複数あります");
	      found = 0;
	    }
	  ...
	  if (found && TREE_TYPE (found) == void_type_node)
	    {
	      error_with_decl (found, "引数`%s'はvoidと宣言されています");
	  ...
	  if (!found)
	    {
	      ...
	      if (extra_warnings)
		warning_with_decl (found, "`%s'は`int'です");
	      pushdecl (found);
	    }
      ...
      for (parm = parmdecls; parm; )
	{
	  ...
	  if (TYPE_SIZE (TREE_TYPE (parm)) == 0)
	    {
	      error_with_decl (parm, "引数`%s'が不完全です");
	      TREE_TYPE (parm) = error_mark_node;
	    }
	      ...
	      if (! comptypes (DECL_ARG_TYPE (parm), TREE_VALUE (type)))
		{
		  if (TREE_TYPE (parm) == TREE_VALUE (type))
			...
			DECL_ARG_TYPE (parm) = integer_type_node;
		      ...
		      if (pedantic)
		        {
			  warning ("引数`%s'は整数格上げ渡し前提です",
				   IDENTIFIER_POINTER (DECL_NAME (parm)));
			  warning ("プロトタイプと一致しません");
			}
		    }
