     ...
     register rtx x;
{
...
#ifdef __human68k__
  {
    if ((!flag_no_sx || flag_pic)
      ...
      {
	rtx regpart = XEXP (x, 0);
	rtx sympart;
	if (GET_CODE (regpart) == SYMBOL_REF)
	  {
	    XEXP (x, 0) = XEXP (x, 1);
	    XEXP (x, 1) = regpart;
	  }
	regpart = XEXP (x, 0);
	sympart = XEXP (x, 1);
	if (!MEM_IN_REMOTE (sympart) && GET_CODE (regpart) == PLUS)
	  {
	    rtx reg0 = XEXP (regpart, 0);
	    rtx reg1 = XEXP (regpart, 1);
	    if (GET_CODE (reg0) == REG && REGNO (reg0) == 13)
	      {
	      	XEXP (x, 1) = reg1;
	      	XEXP (regpart, 1) = sympart;
	      }
	    else if (GET_CODE (reg1) == REG && REGNO (reg1) == 13)
	      {
	      	XEXP (x, 1) = reg0;
	      	XEXP (regpart, 0) = sympart;
	      }
	  }
      }
  }
