...
#ifdef __human68k__
  if (MARIKO_F)
    message ("命令結合を行います 処理命令数 %5d\r", i);
#endif
     ...
     rtx from_insn, to_insn;
{
  ...
  if (code == REG)
    {
      register rtx where_dead = reg_last_death[regno = REGNO (x)];
  ...
  for (i = 0; i < len; i++)
      ...
      if (fmt[i] == 'E')
	{
	  register int j;
	  for (j = XVECLEN (x, i) - 1; j >= 0; j--)
	    regno = move_deaths_2 (XVECEXP (x, i, j), from_cuid, from_insn, to_insn);
	}
      else if (fmt[i] == 'e')
	regno = move_deaths_2 (XEXP (x, i), from_cuid, from_insn, to_insn);
    }
  return regno;
}

     ...
     rtx insn, xprev1, xprev2;
{
  ...
  regno = move_deaths_2 (src2, INSN_CUID (prev2), insn, prev1);

  if (regno > 0
      && reg_n_calls_crossed[regno] == 0
      && reg_n_calls_crossed[REGNO (SET_DEST (pat2))] != 0)
    reg_n_calls_crossed[regno] = reg_n_calls_crossed[REGNO (SET_DEST (pat2))];
