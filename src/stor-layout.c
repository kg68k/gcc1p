...
#ifdef __human68k__
  int record_align = flag_strict_align ?
                       MAX (BITS_PER_UNIT, TYPE_ALIGN (rec))
                     : MAX (STRUCTURE_SIZE_BOUNDARY, TYPE_ALIGN (rec));
#else
     ...
     tree rec;
{
...
#ifdef STRUCTURE_SIZE_BOUNDARY
#ifdef __human68k__
  int union_align = flag_strict_align ? BITS_PER_UNIT : STRUCTURE_SIZE_BOUNDARY;
#else
  ...
  for (field = TYPE_FIELDS (rec); field; field = TREE_CHAIN (field))
	...
	{
	  error_with_decl (field, "`%s'がunionでstaticに宣言されました");
	  TREE_STATIC (field) = 0;
	}
#endif
