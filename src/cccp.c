...
Copyright (C) 1991 06 09 FSHARP Mariko 1.40 Patch ignor VMS modify.
...
int do_define(), do_line(), do_include(), do_undef(), do_error(), do_pragma(),
    do_if(), do_xifdef(), do_else(), do_elif(), do_endif(), do_sccs(),
    do_once();
...
U_CHAR *human68k_getenv();
void human68k_setpath();
void fix_read_buf();
void fix_write_buf();
char *fmt_error;
char *fmt_warn;
...
#ifndef VMS
#ifdef __human68k__
    {&include_defaults[1], "$gcc_include"}, {0, "$include"}
#else
...
#ifndef VMS
#ifdef __human68k__
    {&cplusplus_include_defaults[1], "$cplusplus_include"},
    {&cplusplus_include_defaults[2], "$gcc_include"},
    {0, "$include"}
#else
...
static void pipe_closed() { fatal("output pipe has been closed"); }
...
char **argv;
{
...
#ifdef VMS
  {
    ...
    if (!(s = rindex(progname, ']'))) s = rindex(progname, ':');
    if (s) strcpy(progname, s + 1);
    if (s = rindex(progname, '.')) *s = '\0';
  }
#endif
...
#ifdef SIGPIPE /* for Human68k */
  signal(SIGPIPE, pipe_closed);
#endif

#ifndef VMS
#ifdef __human68k__
  max_include_len = max(max(strlen(cplusplus_include_defaults[0].fname),
                            strlen(cplusplus_include_defaults[1].fname)),
                        strlen(cplusplus_include_defaults[2].fname)) +
                    1;
#else
  max_include_len =
      max(max(sizeof(GCC_INCLUDE_DIR), sizeof(GPLUSPLUS_INCLUDE_DIR)),
  ...
  for (i = 1; i < argc; i++) {
    if (argv[i][0] != '-') {
      ...
      switch (argv[i][1]) {
        ...
        case 'o':
          if (out_fname != NULL) fatal("Output filename specified twice");
          if (i + 1 == argc) fatal("Filename missing after -o option");
          out_fname = argv[++i];
          if (!strcmp(out_fname, "-")) out_fname = "";
          break;
        ...
        case 'W':
          ...
          if (!strcmp(argv[i], "-Wcomments")) warn_comments = 1;
          if (!strcmp(argv[i], "-Wcomment")) warn_comments = 1;
...
#ifdef __human68k__
        case 'f':
          /* -f option  only undump!! */
          include_macro_only = 1;
          break;
#endif

        default:
          fatal("無効なoptionです`%s'", argv[i]);
      }
    }
  }
  ...
  for (i = 1; i < argc; i++)
    if (pend_defs[i]) make_definition(pend_defs[i]);

  /* Do undefines specified with -U.  */
  for (i = 1; i < argc; i++)
    if (pend_undefs[i]) make_undef(pend_undefs[i]);
  ...
  if (print_deps == 0 && (getenv("SUNPRO_DEPENDENCIES") != 0 ||
    ...
    if (deps_stream == 0) pfatal_with_name(output_file);
  }
  ...
  if (print_deps) {
    ...
    if (deps_target) {
      ...
      if (p[len - 2] == '.' &&
        ...
        deps_output(p, len - 2);
      else if (p[len - 3] == '.' && p[len - 2] == 'c' && p[len - 1] == 'c')
        deps_output(p, len - 3);
      else
        deps_output(p, 0);
  ...
  if (!no_trigraphs) trigraph_pcp(fp);
...
#ifdef __human68k__
  {
    ...
    if (!out_fname || !strcmp(out_fname, ""))
      out_fname = "stdout";
    else if ((xfp = fopen(out_fname, "wb")) == 0)
...
#ifdef __human68k__
  fix_write_buf(outbuf.buf, outbuf.bufp - outbuf.buf);
#endif
  ...
  if (print_deps) {
    ...
    if (deps_stream != stdout) {
      fclose(deps_stream);
      if (ferror(deps_stream)) fatal("出力I/Oエラー");
    }
  }

  if (ferror(stdout)) fatal("出力I/Oエラー");
...
trigraph_pcp(buf) FILE_BUF *buf;
{
  ...
  if (bptr != fptr && len > 0) bcopy(fptr, bptr, len);
  buf->length -= fptr - bptr;
  buf->buf[buf->length] = '\0';
  if (warn_trigraphs && fptr != bptr)
    warning("%d個のtrigraph(s)があります", (fptr - bptr) / 2);
}
...
name_newline_fix(bp) U_CHAR *bp;
{
  ...
  if (count == 0 || !is_idchar[*p]) return;

  /* Copy all potentially embarrassing characters
     that follow the backslash-newline pairs
     down to where the pairs originally started.  */

  while (is_idchar[*p]) *bp++ = *p++;
...
rescan(op, output_marks) FILE_BUF *op;
int output_marks;
{
  int kanji_found = 0;
    ...
    *obp++ = c;
    ...
    switch (c) {
      case '\\':
        ...
        if (ident_length > 0) goto specialchar;
        *obp++ = *ibp++;
        break;
      ...
      case '#':
        ...
        if (ip->macro != 0) goto randomchar;
        if (ident_length) goto specialchar;

        /* # keyword: a # must be first nonblank char on the line */
        if (beg_of_line == 0) goto randomchar;
        {
          U_CHAR *bp;
          ...
          while (1) {
            if (is_hor_space[*bp])
              ...
              bp += 2;
              while (!(*bp == '*' && (newline_fix(bp + 1), bp[1]) == '/')) bp++;
              bp += 1;
            } else if (cplusplus && *bp == '/' &&
                       (newline_fix(bp + 1), bp[1]) == '/') {
              bp += 2;
              while (*bp++ != '\n');
            } else
              break;
          }
          if (bp + 1 != ibp) goto randomchar;
        }
          ...
          *obp++ = *ibp;
          switch (*ibp++) {
            case '\n':
              ++ip->lineno;
              ++op->lineno;
              /* Traditionally, end of line ends a string constant with no
                 error. So exit the loop and record the new line.  */
              if (traditional) {
                beg_of_line = ibp;
                goto while2end;
              }
...

        {
          ...
          while (ibp < limit) {
            switch (*ibp++) {
              ...
              case '*':
                if (*ibp == '\\' && ibp[1] == '\n') newline_fix(ibp);
                if (ibp >= limit || *ibp == '/') goto comment_end;
                break;
              case '\n':
                ++ip->lineno;
      ...
      case '$':
        if (!dollars_in_ident) goto randomchar;
        goto letter;
      ...
      case 0:
        if (ibp <= limit) /* Our input really contains a null character.  */
          goto randomchar;
...
ending:
  if (if_stack != ip->if_stack) {
    ...
    error_with_line(line_for_error(if_stack->lineno), "#%sが終わっていません",
                    str);
  }
 ...
 */
FILE_BUF
expand_to_temp_buffer(buf, limit, output_marks) U_CHAR *buf, *limit;
int output_marks;
{
    ...
    if (kt->length == ident_length && !strncmp(kt->name, ident, ident_length)) {
      ...
      while (bp < limit) {
        ...
        switch (c) {
          ...
          case '\'':
          case '\"':
            bp = skip_quoted_string(bp - 1, limit, ip->lineno, &ip->lineno,
                                    &copy_command, &unterminated);
 ...
 */
special_symbol(hp, op) HASHNODE *hp;
FILE_BUF *op;
{
  char *buf;
  time_t t;
  ...
  switch (hp->type) {
    ...
    case T_INCLUDE_LEVEL:
      true_indepth = 0;
      for (i = indepth; i >= 0; i--)
        if (instack[i].fname != NULL) true_indepth++;
    ...
    case T_SPEC_DEFINED:
      ...
      if (!is_idstart[*ip->bufp]) goto oops;
      if (lookup(ip->bufp, -1, -1)) buf = " 1 ";
      while (is_idchar[*ip->bufp]) ++ip->bufp;
      SKIP_WHITE_SPACE(ip->bufp);
      if (paren) {
        if (*ip->bufp != ')') goto oops;
        ++ip->bufp;
      }
      break;
    ...
    oops:
      ...
      error("`defined'の後は`ident'か`(ident)'です");
      break;
...
do_include(buf, limit, op, keyword) U_CHAR *buf, *limit;
FILE_BUF *op;
...
get_filename:
  ...
  switch (*fbeg++) {
    ...
    case '<':
      ...
      if (*fend == '>' && fend + 1 == limit) {
        ...
        if (first_bracket_include) stackp = first_bracket_include;
        break;
      }
      goto fail;
    ...
    fail:
      if (retried) {
        error("#includeの後は\"fname\"か<fname>です");
        return;
      } else {
...
#endif /* VMS */
      if ((f = open(fname, O_RDONLY | O_TEXT, 0666)) >= 0) break;
    }
  }
  ...
  if (f < 0) {
    ...
    if (print_deps > (system_header_p || (system_include_depth > 0))) {
      if (system_header_p)
        warning("存在しないfile <%.*s>は依存関係から外します", fend - fbeg,
                fbeg);
      else {
    ...
    if (system_header_p) system_include_depth++;
#ifdef __human68k__
    if (include_macro_only) no_output++;
#endif
...
nope:
  ...
  if (!success) perror_with_name(fname);
...
do_define(buf, limit, op, keyword) U_CHAR *buf, *limit;
FILE_BUF *op;
struct directive *keyword;
{
  ...
  if (sym_length == 0)
    ...
    msg[sym_length] = 0;
    error("無効なmacro %sです", msg);
  } else {
    if (!strncmp(symname, "defined", 7) && sym_length == 7)
      error("`defined'はマクロにできません");
  }
  ...
  if (*bp == '(') {
    ...
    while (*bp != ')') {
      ...
      arg_ptrs = temp;
      if (!is_idstart[*bp])
        warning("#defineのパラメータが数字から始まっています");
    ...
    if (is_hor_space[*bp]) ++bp; /* skip exactly one blank/tab char */
    /* now everything from bp before limit is the definition. */
...
DEFINITION *d1, *d2;
{
  ...
  if (d1->nargs != d2->nargs) return 1;
  if (strcmp((char *)d1->argnames, (char *)d2->argnames)) return 1;
  for (a1 = d1->pattern, a2 = d2->pattern; a1 && a2;
       a1 = a1->next, a2 = a2->next) {
    if (!((a1->nchars == a2->nchars && !strncmp(p1, p2, a1->nchars)) ||
          !comp_def_part(first, p1, a1->nchars, p2, a2->nchars, 0)) ||
        a1->argno != a2->argno || a1->stringify != a2->stringify ||
        a1->raw_before != a2->raw_before || a1->raw_after != a2->raw_after)
      return 1;
    first = 0;
    p1 += a1->nchars;
    p2 += a2->nchars;
  ...
  if (a1 != a2) return 1;
  if (comp_def_part(first, p1, d1->length - (p1 - d1->expansion), p2,
                    d2->length - (p2 - d2->expansion), 1))
    return 1;
  return 0;
}
...
comp_def_part(first, beg1, len1, beg2, len2, last) int first;
U_CHAR *beg1, *beg2;
int len1, len2;
int last;
{
...
DEFINITION *collect_expansion(buf, end, nargs, arglist)
U_CHAR *buf, *end;
int nargs;
struct arglist *arglist;
{
  ...
  maxsize = (sizeof(DEFINITION) + 2 * (end - limit) + 2 * (p - buf) +
             (limit - p) + 3 + 2);
      ...
      switch (c) {
        ...
        case '#':
          if (p < limit && *p == '#') {
            ...
            exp_p--;
            while (exp_p > lastp && is_hor_space[exp_p[-1]]) --exp_p;
            /* Skip the second #.  */
            p++;
            /* Discard following whitespace.  */
            SKIP_WHITE_SPACE(p);
            concat = p;
            if (limit <= p) error("## operatorがマクロの終わりです");
          } else {
            ...
            if (p == limit || !is_idstart[*p] || nargs <= 0)
              error("# operatorの後はmacro argument nameのはずです");
            else
              stringify = p;
          }
          break;
      ...
      switch (c) {
        ...
        case '\"':
          if (expected_delimiter != '\0') {
            if (c == expected_delimiter) expected_delimiter = '\0';
          } else
    ...
    if (is_idchar[c] && nargs > 0) {
      ...
      if (is_idstart[c]) {
        ...
        for (arg = arglist; arg != NULL; arg = arg->next) {
          ...
          if (arg->name[0] == c && arg->length == id_len &&
  ...
  *exp_p = '\0';
  ...
  if (defn->length + 1 > maxsize) abort();
...
do_line(buf, limit, op, keyword) U_CHAR *buf, *limit;
FILE_BUF *op;
struct directive *keyword;
{
  register U_CHAR *bp;
 ...
 */
do_undef(buf, limit, op, keyword) U_CHAR *buf, *limit;
FILE_BUF *op;
struct directive *keyword;
{
  HASHNODE *hp;
  ...
  if (!strncmp(buf, "defined", 7) && !is_idchar[buf[7]])
    warning("`defined'をundefしています");

  while ((hp = lookup(buf, -1, -1)) != NULL) {
    if (hp->type != T_MACRO) error("undefできません `%s'", hp->name);
    delete_macro(hp);
  }
}
 ...
 */
do_error(buf, limit, op, keyword) U_CHAR *buf, *limit;
FILE_BUF *op;
struct directive *keyword;
{
  int length = limit - buf;
...
do_pragma(buf, limit) U_CHAR *buf, *limit;
{
  while (*buf == ' ' || *buf == '\t') buf++;
  if (!strncmp(buf, "once", 4)) do_once();
}
...
do_sccs() {
  if (pedantic) error("ANSI Cは#sccsは使えません");
}
...
do_if(buf, limit, op, keyword) U_CHAR *buf, *limit;
FILE_BUF *op;
struct directive *keyword;
{
  int value;
...
do_elif(buf, limit, op, keyword) U_CHAR *buf, *limit;
FILE_BUF *op;
struct directive *keyword;
{
  int value;
  ...
  if (if_stack == instack[indepth].if_stack) {
    error("#elifが条件の中ではありません");
    return;
  } else {
    if (if_stack->type != T_IF && if_stack->type != T_ELIF) {
      error("#elifが#elseの後に現れました");
      fprintf(gcc_err_file, " (matches line %d", if_stack->lineno);
      if (if_stack->fname != NULL && ip->fname != NULL &&
          strcmp(if_stack->fname, ip->fname) != 0)
        fprintf(gcc_err_file, ", file %s", if_stack->fname);
 ...
 */
int eval_if_expression(buf, length)
U_CHAR *buf;
int length;
{
  FILE_BUF temp_obuf;
 ...
 */
do_xifdef(buf, limit, op, keyword) U_CHAR *buf, *limit;
FILE_BUF *op;
struct directive *keyword;
{
  int skip;
  ...
  if (end == buf) {
    ...
    if (!traditional)
      warning(
          end == limit ? "#%sに引数がありません" : "#%sが`,'から始まっています",
          keyword->name);
  } else {
    if (pedantic && buf[0] >= '0' && buf[0] <= '9')
      warning("#%sの引数が数字から始まっています");
    else if (end != limit && !traditional)
      warning("#%sの後ろにゴミがあります", keyword->name);
 ...
 */
void conditional_skip(ip, skip, type) FILE_BUF *ip;
int skip;
enum node_type type;
{
  IF_STACK_FRAME *temp;
 ...
 */
void skip_if_group(ip, any) FILE_BUF *ip;
int any;
{
        ...
        for (kt = directive_table; kt->length >= 0; kt++) {
          ...
          if (strncmp(cp, kt->name, kt->length) == 0 &&
            ...
            switch (kt->type) {
              ...
              case T_ELSE:
              case T_ENDIF:
                if (pedantic && if_stack != save_if_stack) validate_else(bp);
              case T_ELIF:
                if (if_stack == instack[indepth].if_stack) {
                  error("#%sが条件の中ではありません", kt->name);
                  break;
                ...
                if (kt->type != T_ENDIF) {
                  if (if_stack->type == T_ELSE)
                    error("#else or #elifが#elseの後に現れました");
 ...
 */
do_else(buf, limit, op, keyword) U_CHAR *buf, *limit;
FILE_BUF *op;
struct directive *keyword;
{
  FILE_BUF *ip = &instack[indepth];
  ...
  if (pedantic) {
    SKIP_WHITE_SPACE(buf);
    if (buf != limit) warning("#elseの後のtextはANSI違反です");
  }

  if (if_stack == instack[indepth].if_stack) {
    error("#elseが条件の中ではありません");
    return;
  } else {
    if (if_stack->type != T_IF && if_stack->type != T_ELIF) {
      error("#elseが#elseの後に現れました");
      fprintf(gcc_err_file, " (matches line %d", if_stack->lineno);
 ...
 */
do_endif(buf, limit, op, keyword) U_CHAR *buf, *limit;
FILE_BUF *op;
struct directive *keyword;
{
  if (pedantic) {
    SKIP_WHITE_SPACE(buf);
    if (buf != limit) warning("#endifの後のtextはANSI違反です");
  }

  if (if_stack == instack[indepth].if_stack)
    error("#endifがアンバランスです");
  else {
...
validate_else(p) register U_CHAR *p;
{
  ...
  if (*p && *p != '\n') warning("#elseか#endifの後のtextはANSI違反です");
}
 ...
 */
U_CHAR *skip_to_end_of_comment(ip, line_counter)
register FILE_BUF *ip;
 ...
 */
U_CHAR *skip_quoted_string(bp, limit, start_line, count_newlines,
                           backslash_newlines_p, eofp)
register U_CHAR *bp;
register U_CHAR *limit;
int start_line;
int *count_newlines;
...
int *eofp;
{
  ...
  while (1) {
    if (bp >= limit) {
      error_with_line(line_for_error(start_line), "文字列が終わっていません");
      if (eofp) *eofp = 1;
      break;
    }
    c = *bp++;
    if (c == '\\') {
      while (*bp == '\\' && bp[1] == '\n') {
        if (backslash_newlines_p) *backslash_newlines_p = 1;
        if (count_newlines) ++*count_newlines;
        bp += 2;
      }
      if (*bp == '\n' && count_newlines) {
        if (backslash_newlines_p) *backslash_newlines_p = 1;
...
void output_line_command(ip, op, conditional, file_change) FILE_BUF *ip, *op;
int conditional;
...
#ifdef __human68k__
  {
    register char *s, *t = ip->fname;
    register int x;

    sprintf(line_cmd_buf, "# %d \"", ip->lineno);
    s = line_cmd_buf + strlen(line_cmd_buf);
    while (x = *t++) {
      if (x == '\\') *s++ = x;
      *s++ = x;
    }
...
void macroexpand(hp, op) HASHNODE *hp;
FILE_BUF *op;
{
  int nargs;
  ...
  if (nargs >= 0) {
    ...
    for (i = 0; i < nargs; i++) {
      ...
      args[i].raw_length = args[i].expand_length = args[i].stringified_length =
          0;
    ...
    if (nargs == 0 && i > 0)
      error("macro `%s'に引数があります", hp->name);
    else if (i < nargs) {
      /* traditional C allows foo() if foo wants one argument.  */
      if (nargs == 1 && i == 0 && traditional)
        ;
      else if (i == 0)
        error("macro `%s'に引数がありません", hp->name);
      else if (i == 1)
        error("macro `%s'の引数1個です", hp->name);
      else
        error("macro `%s'の引数は%d個です", hp->name, i);
    } else if (i > nargs)
      error("(%d)個の引数はmacro `%s'には多すぎます", i, hp->name);
    ...
    if (nargs == 0) {
      ...
      for (ap = defn->pattern; ap != NULL; ap = ap->next) {
        ...
        for (i = 0; i < ap->nchars; i++) xbuf[totlen++] = exp[offset++];
        ...
        if (ap->stringify != 0) {
          ...
          int c;
          i = 0;
          while (i < arglen && (c = arg->raw[i], is_space[c])) i++;
          while (i < arglen && (c = arg->raw[arglen - 1], is_space[c]))
            arglen--;
          if (!traditional) xbuf[totlen++] = '\"'; /* insert beginning quote */
          for (; i < arglen; i++) {
            c = arg->raw[i];
            ...
            if (isprint(c))
              ...
              xbuf[totlen++] = c;
#ifdef __human68k__
            else if (in_string && iskanji(c)) {
              xbuf[totlen++] = c;
              xbuf[totlen++] = arg->raw[++i];
            }
#endif
            else {
          ...
          if (!traditional) xbuf[totlen++] = '\"'; /* insert ending quote */
        } else if (ap->raw_before || ap->raw_after || traditional) {
          U_CHAR *p1 = arg->raw;
          U_CHAR *l1 = p1 + arg->raw_length;
          if (ap->raw_before) {
            while (p1 != l1 && is_space[*p1]) p1++;
            while (p1 != l1 && is_idchar[*p1]) xbuf[totlen++] = *p1++;
        ...
        if (totlen > xbuf_len) abort();
      }

      /* if there is anything left of the definition
         after handling the arg list, copy that in too. */

      for (i = offset; i < defn->length; i++) xbuf[totlen++] = exp[i];

      xbuf[totlen] = 0;
      xbuf_len = totlen;

      for (i = 0; i < nargs; i++) {
        if (args[i].free1 != 0) free(args[i].free1);
        if (args[i].free2 != 0) free(args[i].free2);
      }
    }
  } else {
    xbuf = defn->expansion;
...
register struct argdata *argptr;
{
  ...
  /* Try to parse as much of the argument as exists at this
     input stack level.  */
  U_CHAR *bp =
      macarg1(ip->bufp, ip->buf + ip->length, &paren, &newlines, &comments);
  ...
  if (!(ip->fname != 0 && (newlines != 0 || comments != 0)) &&
    ...
    if (argptr != 0) {
      ...
      if ((newlines || comments) && ip->fname != 0)
        argptr->raw_length =
            final_start + discard_comments(argptr->raw + final_start,
                                           argptr->raw_length - final_start,
                                           newlines);
      argptr->raw[argptr->raw_length] = 0;
      if (argptr->raw_length > bufsize + extra) abort();
    }
  }
  ...
  if (argptr != 0) {
    ...
    obuf =
        expand_to_temp_buffer(argptr->raw, argptr->raw + argptr->raw_length, 1);
    ...
    if (!traditional) {
      while (buf != lim && is_space[*buf]) buf++;
      while (buf != lim && is_space[lim[-1]]) lim--;
    }
...
U_CHAR *macarg1(start, limit, depthptr, newlines, comments)
U_CHAR *start;
register U_CHAR *limit;
...
int discard_comments(start, length, newlines)
U_CHAR *start;
int length;
int newlines;
{
    ...
    *obp++ = c = *ibp++;
    switch (c) {
      ...
      case '/':
        ...
        if (ibp[0] != '*' || ibp + 1 >= limit) break;
        obp--;
        ibp++;
        while (ibp + 1 < limit) {
          if (ibp[0] == '*' && ibp[1] == '\\' && ibp[2] == '\n')
            newline_fix(ibp + 1);
          if (ibp[0] == '*' && ibp[1] == '/') break;
          ibp++;
        }
        ibp += 2;
        break;
...
#ifdef __human68k__
static void tag_file_open() {
  char *temp_dir = getenv("TEMP");
  char *mariko = getenv("GCC_OPTION0");

  if (!temp_dir && !(temp_dir = getenv("temp"))) temp_dir = "";
  ...
  if (mariko) {
    for (; *mariko; mariko++)
      if (*mariko == 'D') {
        char *p;
        char *name;
        p = name = alloca(strlen(temp_dir) + 14);
 ...
 */
error(msg, arg1, arg2, arg3) char *msg;
{
  int i;
  FILE_BUF *ip = NULL;

  if (inhibit_warnings) return 0;
...
#ifdef __human68k__
  if (errors == 0) tag_file_open();
  if (tagfile) {
    if (ip != NULL) fprintf(tagfile, fmt_error, ip->fname, ip->lineno);
    fprintf(tagfile, msg, arg1, arg2, arg3);
    fprintf(tagfile, "\n");
  }
#endif
  if (ip != NULL) fprintf(gcc_err_file, fmt_error, ip->fname, ip->lineno);
  fprintf(gcc_err_file, msg, arg1, arg2, arg3);
  fprintf(gcc_err_file, "\n");
  errors++;
  return 0;
}
...
error_from_errno(name) char *name;
{
  int i;
  FILE_BUF *ip = NULL;
  extern int errno;
...
warning(msg, arg1, arg2, arg3) char *msg;
{
  int i;
  FILE_BUF *ip = NULL;
  ...
  if (ip != NULL) fprintf(gcc_err_file, fmt_warn, ip->fname, ip->lineno);
...
error_with_line(line, msg, arg1, arg2, arg3) int line;
char *msg;
{
  int i;
  FILE_BUF *ip = NULL;
...
#endif
  if (ip != NULL) fprintf(gcc_err_file, fmt_error, ip->fname, line);
  fprintf(gcc_err_file, msg, arg1, arg2, arg3);
  fprintf(gcc_err_file, "\n");
  errors++;
  return 0;
...
int line;
{
  ...
  for (i = indepth; i >= 0;) {
    if (instack[i].fname != 0) return line1;
    i--;
    if (i < 0) return 0;
...
register int needed;
{
  ...
  if (obuf->length - (obuf->bufp - obuf->buf) > needed) return;
  ...
  if (minsize > obuf->length) obuf->length = minsize;

  if ((p = (U_CHAR *)xrealloc(obuf->buf, obuf->length)) == NULL) memory_full();
 ...
 */
HASHNODE *install(name, len, type, value, hash)
U_CHAR *name;
int len;
enum node_type type;
int value;
...
/* watch out here if sizeof (U_CHAR *) != sizeof (int) */
{
  ...
  if (hash < 0) hash = hashf(name, len, HASHSIZE);
  ...
  hashtab[bucket] = hp;
  hp->prev = NULL;
  if (hp->next != NULL) hp->next->prev = hp;
  hp->type = type;
  hp->length = len;
  hp->value.ival = value;
...
delete_macro(hp) HASHNODE *hp;
{
  if (hp->prev != NULL) hp->prev->next = hp->next;
  if (hp->next != NULL) hp->next->prev = hp->prev;

  /* make sure that the bucket chain header that
     the deleted guy was on points to the right thing afterwards. */
  if (hp == *hp->bucket_hdr) *hp->bucket_hdr = hp->next;
    ...
    for (hp = hashtab[bucket]; hp; hp = hp->next) {
      if (hp->type == T_MACRO) {
        ...
        for (ap = defn->pattern; ap != NULL; ap = ap->next) {
          ...
          if (ap->nchars != 0) concat = 0;
          offset += ap->nchars;
          if (ap->stringify) printf(" #");
          if (ap->raw_before && !concat) printf(" ## ");
          concat = 0;
...
dump_defn_1(base, start, length) U_CHAR *base;
int start;
int length;
{
  U_CHAR *p = base + start;
...
dump_arg_n(defn, argnum) DEFINITION *defn;
int argnum;
{
...
initialize_char_syntax() {
  ...
  /* kanji 2nd byte */
  for (i = 0x80; i <= 0xfc; i++) is_idchar[i] = 1;
...
initialize_builtins() {
  ...
  if (!traditional) install("__STDC__", -1, T_CONST, STDC_VALUE, -1);
 ...
 */
make_definition(str) U_CHAR *str;
{
  FILE_BUF *ip;
  struct directive *kt;
  U_CHAR *buf, *p;
...
deps_output(string, size) char *string;
int size;
{
...
#ifndef __LIBC__
void bzero(b, length) register char *b;
register int length;
{
#ifdef VMS
  short zero = 0;
...
void bcopy(b1, b2, length) register char *b1;
register char *b2;
register int length;
{
#ifdef VMS
...
int bcmp(b1, b2, length) /* This could be a macro! */
register char *b1;
register char *b2;
register int length;
{
...
void fatal(str, arg) char *str, *arg;
{
  fprintf(gcc_err_file, "%s: ", progname);
  fprintf(gcc_err_file, str, arg);
  fprintf(gcc_err_file, "\n");
  exit(FATAL_EXIT_CODE);
}
...
void memory_full() { fatal("Memory exhausted."); }
...
int file_size_and_mode(fd, mode_pointer, size_pointer)
int fd;
int *mode_pointer;
...
hack_vms_include_specification(fname) char *fname;
{
  ...
  /* Ignore leading "./"s */
  while (fname[0] == '.' && fname[1] == '/') strcpy(fname, fname + 2);
...
void fix_read_buf(buf, count) U_CHAR *buf;
int count;
{
  while (--count >= 0) {
    U_CHAR high = *buf++;
    if (iskanji(high)) {
      ...
      if (high == 0x80 || (0xf0 <= high && high <= 0xf3)) *buf += 0x80 - '0';
      buf++;
    }
  }
}

void fix_write_buf(buf, count) U_CHAR *buf;
int count;
{
  while (--count >= 0) {
    U_CHAR high = *buf++;
    if (iskanji(high)) {
      ...
      if (high == 0x80 || (0xf0 <= high && high <= 0xf3)) {
        if (0x80 <= low && low <= 0x89) buf[-1] = tbl[low & 0x0f];
      } else if ('0' <= low && low <= '9')
        buf[-1] = tbl[low & 0x0f];
    }
  }
...
U_CHAR *human68k_getenv(s)
register unsigned char *s;
{
  extern char **environ;
  register unsigned char **e, *p, *x;
  register int c;

  for (e = (unsigned char **)environ; p = *e; e++) {
    for (x = s; TOLOWER(*x) == TOLOWER(*p); x++, p++);
    if (*x == 0 && *p == '=') return ++p;
  }
  return 0;
}

void human68k_setpath(d) register struct file_name_list *d;
{
