     ...
     register tree fndecl;
{
  ...
  if (last && TREE_VALUE (last) != void_type_node)
    return "可変長引数関数はinlineにできません";

  if (current_function_calls_alloca)
    return "allocaを使う関数はinlineにできません";

  /* If its not even close, don't even look.  */
  if (!TREE_INLINE (fndecl) && get_max_uid () > 3 * max_insns)
    return "inlineにするには大きすぎる関数です";
  ...
  for (parms = DECL_ARGUMENTS (fndecl); parms; parms = TREE_CHAIN (parms))
	...
	return "構造をもつ引数の関数はinlineにできません";
      if (last == NULL_TREE && TREE_ADDRESSABLE (parms))
	return "プロトタイプがなく、引数のアドレスをとる関数はinlineにできません";
	...
	return "引数のアドレスを使う関数はinlineにできません";
    }
    ...
    case CALL:
	 ...
	 if function-address cse isn't wanted or if we won't do cse.  */
      ...
      if (! (optimize
	     ...
	     && MEM_IN_REMOTE (XEXP (XEXP (orig, 0), 0)))
          )
#else
