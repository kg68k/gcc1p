...
extern int flag_strings_align;
extern int bsr_seen;
extern int flag_all_short;
extern int flag_text;
...
#ifndef __human68k__
#define EXTRA_SECTIONS bss_section
...
#define SELECT_SECTION(exp) select_section(exp)
#endif
...
offset_section ()
{
  if (in_section != in_offset)
    {
      ASM_OUTPUT_ALIGN (asm_out_file, 1);
      fputs ("\t.offset\t0\n", asm_out_file);
      in_section = in_offset;
    }
}
...
rldata_section ()
{
  if (in_section != in_rldata)
    {
      ASM_OUTPUT_ALIGN (asm_out_file, 1);
      fputs ("\t.rldata\n", asm_out_file);
      in_section = in_rldata;
    }
}
     ...
     tree exp;
{
  ...
  if (flag_text || TREE_PCR_DATA (exp))
    {
      text_section ();
      return;
    }
  if (TREE_RELOCATE (exp))
    {
      offset_section ();
      return;
    }
  if (TREE_COMMON (exp))
    if (DECL_INITIAL (exp) && DECL_INITIAL (exp) != error_mark_node)
      {
      	data_section ();
      	return;
      }
  ...
  else if (DECL_INITIAL (exp) && DECL_INITIAL (exp) != error_mark_node)
      {
      	rdata_section ();
      	return;
      }
     ...
     tree decl;
{
  ...
  if (DECL_RTL (decl) == 0)
	...
	= gen_rtx (MEM, DECL_MODE (decl),
		   temp=gen_rtx (SYMBOL_REF, Pmode, DECL_ASSEMBLER_NAME (decl)));
      if (TREE_DOSCALL (decl))
	MEM_IN_REMOTE (temp) = 0;
      else
	MEM_IN_REMOTE (temp) = 1;
      if (flag_all_short)
        {
          MEM_IN_PCR (temp) = 1;
          MEM_MAY_IN_PCR (temp) = 1;
        }
    }
...
static char frame_oder;
static char reg_ret_regs[FIRST_PSEUDO_REGISTER];
int
     ...
     int top_level;
{
  ...
  /* duplicate declaration, setup for flag-all-remote */
  if (flag_remote && TREE_CODE (decl) != FUNCTION_DECL)
    TREE_REMOTE (decl) = 1;
  ...
  if (DECL_RTL (decl) == 0
	...
	error_with_decl (decl,
			 "`%s'はレジスタ変数ではありません");
      else if (reg_number > 16 && ! TARGET_68881)
	error_with_decl (decl,
			 "レジスタ変数`%s'は68881指定が必要です");
      else if (TREE_REGDECL (decl) && TREE_CODE (decl) == FUNCTION_DECL)
	error ("関数はregister宣言できません");
      else if (TREE_REGDECL (decl) && TYPE_MODE (TREE_TYPE (decl)) == BLKmode)
	error_with_decl (decl, "`%s'はregister宣言できないタイプです");
      /* Now handle properly declared static register variables.  */
      else if (TREE_REGDECL (decl))
	{
	  int nregs;
	  if (pedantic)
	    warning ("ANSI Cではregister名指定変数は使えません");
	  if (DECL_INITIAL (decl) != 0 && top_level)
	    {
	      DECL_INITIAL (decl) = 0;
	      error ("global register変数は初期化できません");
	    }
	  if (fixed_regs[reg_number] == 0
	      && function_defined && top_level)
	    error ("global register変数は関数定義の後に宣言できません");
	  if (frame_oder) {
	    extern int mariko_forever;
	    if (DECL_MODE (decl) != SImode)
	      error_with_decl (decl,"frame pointerにできないタイプです");
	    mariko_frame_pointer = reg_number;
	    REGNO (frame_pointer_rtx) = reg_number;
	    if (top_level) mariko_forever = reg_number;
	  }
	  if ((!flag_no_sx || flag_pic) && reg_number == 13)
	    error_with_decl (decl,"``a5''は使えません");
	  ...
	  while (nregs > 0)
	    if (top_level)
	      ...
	      extern char can_not_optimize_regs[];
	      reg_number += --nregs;
	      can_not_optimize_regs[reg_number] = 1;
	      inter_use_regs[reg_number] = reg_ret_regs[reg_number];
	      if (global_regs[reg_number] && !inter_use_regs[reg_number])
		warning("global registerが関数内で再使用されています");
	    }
	  if (top_level) init_reg_sets_1 ();
	}
      ...
      if (DECL_RTL (decl) == 0)
	{
	  ...
	  if (DECL_INITIAL (decl) && TREE_RELOCATE (decl))
	    {
	      error ("relocate は初期化できません");
	      DECL_INITIAL (decl) = 0;
	    }
	  ...
	  if (!top_level && !TREE_EXTERNAL (decl) && asmspec == 0)
	    {
	      ...
	      if (TREE_RELOCATE (decl))
		error ("relocate は関数内部で宣言できません");
	  ...
	  if (TREE_CODE (decl) != FUNCTION_DECL)
	    {
	      temp = gen_rtx (SYMBOL_REF, Pmode, name);
	      if (TREE_REMOTE (decl))
	        MEM_IN_REMOTE (temp) = 1;
	      if (TREE_PCR_DATA (decl))
	        {
	          MEM_IN_PCR (temp) = 1;
	          DECL_RTL (decl) = gen_rtx (MEM,
	                                     DECL_MODE (decl),
	                                     temp);
	        }
	      else if (TREE_RELOCATE (decl))
	        {
	          if (TREE_REMOTE (decl))
	            MEM_IN_REMOTE (temp) = 1;
	          DECL_RTL (decl) = gen_rtx (MEM, DECL_MODE (decl), temp);
	        }
	      else if (TREE_COMMON (decl))
	        {
	          MEM_IN_REMOTE (temp) = 1;
		  DECL_RTL (decl) = gen_rtx (MEM, DECL_MODE (decl), temp);
		}
	        ...
	        DECL_RTL (decl) = gen_rtx (MEM, DECL_MODE (decl),
	      				   gen_rtx (PLUS, SImode, sx_greg, temp));
	    }
	  else
	    {
	      DECL_RTL (decl) = gen_rtx (MEM, DECL_MODE (decl),
     ...
     int at_end;
{
...
#ifdef SDB_DEBUGGING_INFO
#ifdef __human68k__
  if (write_symbols == SDB_DEBUG && !TREE_SYM_WRITTEN (decl) && top_level)
    {
  ...
  if (TREE_COMMON (decl) || TREE_RELOCATE (decl) || TREE_PCR_DATA (decl))
    name = (char *) XEXP (DECL_RTL (decl), 0);
#if 0
  else if (TREE_PCR_DATA (decl))
    name = (char *) XEXP (XEXP (XEXP (DECL_RTL (decl), 0), 0) ,1);
#endif
  else
    name = (char *) XEXP (XEXP (DECL_RTL (decl), 0) ,1);

  if (GET_CODE ((rtx) name) != SYMBOL_REF)
    return;

  name = XSTR ((rtx) name, 0);
     ...
     int size;
{
  ...
  ASM_GENERATE_INTERNAL_LABEL (name, "?F", const_labelno);
  ++const_labelno;
  ...
  {
    extern int flag_remote;
    rtx def = gen_rtx (SYMBOL_REF, Pmode, namestring);
    if (flag_no_sx || flag_remote)
      MEM_IN_REMOTE (def) = 1;
    else
      MEM_IN_REMOTE (def) = 0;
    x = (flag_no_sx && !flag_pic) ? def : gen_rtx (PLUS, SImode, sx_greg, def);
  }
#endif
  ASM_OUTPUT_LOCAL (asm_out_file, name, size, rounded);
  return x;
...
#else
#ifdef __human68k__
if (flag_strings_align)
  align = (TREE_CODE(exp) == STRING_CST) ? ( TREE_CODE(TREE_TYPE(exp))==ARRAY_TYPE
          ? 16 : TYPE_ALIGN(TREE_TYPE (exp) )) : TYPE_ALIGN(TREE_TYPE(exp));
else  align = TYPE_ALIGN (TREE_TYPE (exp));
#else
  ...
  ASM_OUTPUT_INTERNAL_LABEL (asm_out_file, "?C", const_labelno);
  ...
  ASM_GENERATE_INTERNAL_LABEL (label, "?C", const_labelno);
  ...
  if ((TREE_CODE (exp) == STRING_CST) && !flag_writable_strings)
    {
      if (flag_all_short)
        ...
        MEM_MAY_IN_PCR (def) = 1;
    }
  if (flag_writable_strings)
    {
      extern int flag_remote;
      if ((flag_no_sx && !flag_pic) || flag_remote)
        MEM_IN_REMOTE (def) = 1;
      else
        MEM_IN_REMOTE (def) = 0;
      def = (flag_no_sx && !flag_pic ) ? def : gen_rtx (PLUS, SImode, sx_greg, def);
    }
  else
    MEM_IN_REMOTE (def) = 1;  
#endif
