/* Move constant computations out of loops.
   ...
   Copyright (C) 1991 06 10 FSHARP Mariko
		  ...
		  if (temp = find_reg_note (p, REG_RETVAL
					    , 0))
		    {
		      ...
		      if (INSN_DELETED_P (p))
		        {
		          p = XEXP (temp, 0);
		          p = find_reg_note (p, REG_LIBCALL, 0);
		          if (!p)
		            abort ();
			  p = XEXP (p, 0);
		        }
		  ...
		  if (temp = find_reg_note (i1, REG_RETVAL, 0))
		    {
		      ...
		      XEXP (temp, 0) = i1;
#else
		      rtx tp = find_reg_note (first, REG_LIBCALL, 0);
		      if (tp)
		        {
			  XEXP (temp, 0) = first;
			  XEXP (tp, 0) = i1;
...
#ifdef __human68k__
		    else if (find_reg_note (m1->insn, REG_LIBCALL, 0))
                         goto skip;
		    delete_insn (m1->insn);
		skip:               ;
#else
      ...
      for (p = entry_insn; p != endtest; p = NEXT_INSN (p))
        {
         ...
         if (!inline_mult_ok)
           break;
         if (GET_CODE (p) == INSN && (tem = find_reg_note (p, REG_LIBCALL, 0)))
           {
             tem = find_reg_note (XEXP (tem, 0), REG_EQUAL, 0);
             if (tem)
               {
		 tem = XEXP (tem, 0);
		 if (GET_CODE (tem) == MULT
     ...
     register rtx x;
{
  ...
  switch (code)
	...
	return 1;
#ifdef __human68k__
      if ((!flag_no_sx || flag_pic) && REGNO (x) == 13)
        return 1;
#endif
		  ...
		  if (CONSTANT_P (src)
			  ...
			  && ! reg_set_between_p (src, p, loop_start)))
		    {
		      ...
		      if (note)
		        {
		          note = XEXP (note, 0);
		          if (GET_CODE (note) == ZERO_EXTEND
		              && GET_CODE (XEXP (note, 0)) == REG
		              && REGNO (XEXP (note, 0)) == REGNO (src)
		              && GET_CODE (SET_DEST (PATTERN (p))) == STRICT_LOW_PART)
		            class_struct[dest_regno]->initial_value = dest;
		          else
