...
static int
mult_search(int number, int offset)
{
  int	twoi;
  int	i;
  int	rest0, rest1;
  ...
  if (number >= twoi) {
    coef[offset] = 1;
    return mult_search(number - twoi, offset - 1) + 1;
  }
  if (number <= (twoi >> 1)) {
    coef[offset] = 0;
    return mult_search(number, offset - 1);
  }
  rest0 = mult_search(number, offset - 1);
  rest1 = mult_search(twoi - number, offset - 1) + 1;
  if (rest1 < rest0) {
    rest0 = rest1;
    coef[offset] = 1;
    for (i = 0; i < offset; ++i)
...
/* this is Dummy */
void init_mult_expand()
{
}

extern int inline_mult_ok;

static rtx
expand_mult_inline (mode, op0, op1, target, unsignedp)
     enum machine_mode mode;
     ...
     int unsignedp;
{
  ...
  if (GET_CODE (op1) == CONST_INT)
      ...
      if (foo >= 0)
	{
	  ...
	  for ( i = sif ; i >= 0 ; i--)
	    {
	      if ( coef[i] != 0 ) {
		if (tem) {
		  enum rtx_code code = coef[i]==1 ? PLUS : MINUS;
		  tem =
		    force_operand ( gen_rtx ( code, mode,
					     expand_mult_inline ( mode, tem,
								 gen_rtx ( CONST_INT, mode, 1 << sif-i), 0, 0), op0)
				   ,0);
		  sif -= (sif-i);
		} else {
		  tem = op0;
	  ...
	  if ( sif > 0 )
	    tem =	expand_mult_inline ( mode, tem,
					    gen_rtx ( CONST_INT, mode, 1 << sif ), 0, 0);
	  tem = negate ? force_operand ( tem, target) : tem;
	  tem = negate ? expand_unop (mode, neg_optab, tem, target, 0) : tem;
	  return tem;
	}
    }
  return 0;
     ...
     int unsignedp;
{
  ...
  if (GET_CODE (op1) == CONST_INT)
      ...
      if (bar >= 0 && foo >= 0)
	{
      ...
      else if (!TARGET_68020 && inline_mult_ok)
	  {
	    rtx insn_last;
	    rtx insn_first;
	    rtx tem;
	    op0 = protect_from_queue (op0, 0);
	    insn_first = get_last_insn();
	    force_reg (mode,op1);
	    tem = op0;
	    tem  = expand_mult_inline (mode, tem, op1, target, unsignedp);
	    if (tem)
	      {
		rtx list;
		insn_first = insn_first ? NEXT_INSN (insn_first) : get_insns();
