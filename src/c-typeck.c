/* Build expressions with type checking for C compiler.
   ...
   Copyright (C) 1991 06 09 FSHARP Mariko modify Errmesage
     ...
     tree type;
{
  ...
  if (value != 0 && (TREE_CODE (value) == VAR_DECL
	...
	{
	case RECORD_TYPE:
	  errmsg = "`struct %s'は未定義です";
	  break;

	case UNION_TYPE:
	  errmsg = "`union %s'は未定義です";
	  break;

	case ENUMERAL_TYPE:
	  errmsg = "`enum %s'は未定義です";
	  break;
     ...
     tree type;
{
  ...
  if (code == FUNCTION_TYPE)
	...
	warning ("sizeofが関数に使われています");
      return build_int (1);
    }
  if (code == VOID_TYPE)
    {
      if (pedantic || warn_pointer_arith)
	warning ("sizeofがvoid typeに使われています");
      return build_int (1);
    }
     ...
     tree array, index;
{
  ...
  if (TREE_CODE (TREE_TYPE (array)) == ARRAY_TYPE
	...
	warning ("ANSI Cは非左辺値配列の添え字を許しません");
...
#ifdef __human68k__
      TREE_DOSCALL (name) = TREE_DOSCALL (function);
#endif
      ...
      if (type != 0)
	{
	  ...
	  if (TYPE_SIZE (type) == 0)
		...
		type = integer_type_node;
	      ...
	      if (name)
		{
		  char warnbuf[32 + strlen (IDENTIFIER_POINTER (name))]; 
		  sprintf (warnbuf, "関数`%s' %d 番目の引数",
			   IDENTIFIER_POINTER (name), arg_pos);
		  if (TREE_DOSCALL (name))
		    {
		      if (typetail)
		        type =  TREE_VALUE (typetail);
		    }
		  parmval = convert_for_assignment (type, val, warnbuf);
		}
	      else
		parmval = convert_for_assignment (type, val, "引数");
#else
	      parmval = convert_for_assignment (type, val, "引数");
#endif
#ifdef PROMOTE_PROTOTYPES
  ...
  switch (code)
      ...
      else if (code0 == POINTER_TYPE && code1 == POINTER_TYPE)
	{
	  ...
	  if (comp_target_types (dt0, dt1))
		...
		warning ("ANSI Cはvoid *と関数ポインタの比較を許しません");
	    }
	...
	{
	  if (! comp_target_types (dt0, dt1))
	    warning ("キャストなしで異なるポインタを比較しました");
	  else if (pedantic 
		   && TREE_CODE (TREE_TYPE (dt0)) == FUNCTION_TYPE)
	    warning ("ANSI Cはポインタと関数を比較することを許していません");
      ...
      else if (code0 == POINTER_TYPE && code1 == POINTER_TYPE)
	{
	  if (! comp_target_types (dt0, dt1))
	    warning ("異種ポインタを比較しています");
		   ...
		   && TREE_CODE (TREE_TYPE (dt0)) == FUNCTION_TYPE)
	    warning ("ANSI Cではポインタと関数ヘのポインタを比較できません");
	  result_type = integer_type_node;
	}
      else if (code0 == POINTER_TYPE && TREE_CODE (op1) == INTEGER_CST
		&& integer_zerop (op1))
     ...
     register tree ptrop, intop;
{
  ...
  if (TREE_CODE (TREE_TYPE (result_type)) == VOID_TYPE)
    {
      if (pedantic || warn_pointer_arith)
	warning ("void ポインタを算術演算しています");
      size_exp = integer_one_node;
    }
  else if (TREE_CODE (TREE_TYPE (result_type)) == FUNCTION_TYPE)
    {
      if (pedantic || warn_pointer_arith)
	warning ("関数へのポインタを算術演算しています");
      size_exp = integer_one_node;
    }
  else
     ...
     register tree op0, op1;
{
  ...
  if (pedantic)
	...
	warning ("voidポインタを減算に用いています");
      if (TREE_CODE (TREE_TYPE (dt0)) == FUNCTION_TYPE)
	warning ("関数へのポインタを減算に用いています");
    }
...
#else
      if (extra_warnings || (max_lt && !unsignedp0))
#endif
	{
     ...
     int noconvert;
{
  ...
  switch (code)
	...
	case FIX_CEIL_EXPR:
	  if (pedantic)
	    warning ("ANSI Cはキャスト表現のアドレスは許していません");
	...
	if (TREE_CODE (arg) == COMPONENT_REF)
	  {
	    ...
	    if (TREE_PACKED (field))
	      {
		error ("bit-fieldのメンバ`%s'のアドレスは参照できません",
     ...
     tree expr;
{
  ...
  if (form == MODIFY_EXPR)
    {
      form = TREE_CODE (TREE_OPERAND (expr, 1));
      if (form == INTEGER_CST
	  || form == STRING_CST)
	warning ("条件式で定数を代入しています");
      if (form == TRUTH_ANDIF_EXPR || form == TRUTH_ORIF_EXPR
	  || form == TRUTH_AND_EXPR || form == TRUTH_OR_EXPR
	  || form == TRUTH_NOT_EXPR
	  || form == EQ_EXPR || form == NE_EXPR
	...
	if (TREE_REGDECL (x) && !TREE_ADDRESSABLE (x))
	  {
	    if (TREE_PUBLIC (x))
	      {
		error ("レジスタ変数`%s'のアドレスを求めています",
		       IDENTIFIER_POINTER (DECL_NAME (x)));
		return 0;
	      }
	    warning ("レジスタ変数`%s'のアドレスを求めています",
		     IDENTIFIER_POINTER (DECL_NAME (x)));
	  }
     ...
     tree ifexp, op1, op2;
{
  ...
  if (type1 == type2)
	...
	warning ("ANSI Cはvoid sideだけの条件式は許されていません");
      ...
      else if (TYPE_MAIN_VARIANT (TREE_TYPE (type1)) == void_type_node)
	{
	  if (pedantic && TREE_CODE (type2) == FUNCTION_TYPE)
	    warning ("ANSI Cはvoid *と関数へのポインタとの条件式は許していません");
	  result_type = qualify_type (type1, type2);
	}
	...
	warning ("条件式にpointer/integer双方存在します");
      else
	{
  ...
  if (type == TREE_TYPE (value))
      ...
      if (pedantic)
	{
	  if (TREE_CODE (type) == RECORD_TYPE
	    ...
	    warning ("ANSI Cは構造体を構造体にキャストすることは許していません");
	}
    }
  else
    {
     ...
     enum tree_code modifycode;
{
  ...
  newrhs = convert_for_assignment (lhstype, newrhs, "代入");
  ...
  if (olhstype == TREE_TYPE (result))
    return result;
  return convert_for_assignment (olhstype, result, "代入");
}
  ...
  /* Conversions among pointers */
	      ...
	      == unsigned_type (TYPE_MAIN_VARIANT (ttr))))
	{
	  if (pedantic
	    ...
	    warning ("%sは異種のポインタです", errtype);
	  else
	    {
	      if (! TREE_READONLY (ttl) && TREE_READONLY (ttr))
		warning ("%sでconstポインタをコピーしています", errtype);
	      if (! TREE_VOLATILE (ttl) && TREE_VOLATILE (ttr))
		warning ("%sでvolatileポインタをコピーしています", errtype);
	    }
	}
      else
	warning ("%sは異種のポインタです", errtype);
	...
	{
	  warning ("%sで整数からポインタにしました", errtype);
	  return convert (type, rhs);
	}
    ...
    {
      warning ("%sでポインタから整数にしました", errtype);
      return convert (type, rhs);
    }

  error ("%sは異なったタイプです", errtype);
  return error_mark_node;
}

     ...
     tree type, init, *tail;
{
  ...
  if (element && (TREE_TYPE (element) == type
	...
	warning ("struct,unionでない初期化式に`{}'があります");
  ...
  for (i = 0, tail = outputs; tail; tail = TREE_CHAIN (tail), i++)
      ...
      else
	{
	  ...
	  if (TREE_READONLY (o[i])
	    ...
	    readonly_warning (o[i], "`asm'による変換");
	}
    }
     ...
     tree retval;
{
  ...
  if (TREE_THIS_VOLATILE (current_function_decl))
    warning ("volatile宣言された関数にreturnがあります");

  if (!retval)
    {
      current_function_returns_null = 1;
      if (warn_return_type && valtype != 0 && TREE_CODE (valtype) != VOID_TYPE)
	warning ("voidでない関数に値のないreturnがあります");
      expand_null_return ();
    }
	...
	warning ("voidである関数に値を持ったreturnがあります");
      expand_return (retval);
    }
  else
    {
      tree t = convert_for_assignment (valtype, retval, "戻り値");
