/* Convert tree expression to rtl instructions, for GNU compiler.
   ...
   Copyright (C) 1991 FSHARP Mariko intterupt function expand
...
#ifdef __human68k__
int flag_call_seen = 0;
rtx sx_ret_regs;
rtx befor_sx_call_val;
#endif
...
#ifdef __human68k__
  /* gcc extention allowed
     struct foo *p;
     *p ++ = (struct foo) { ...};
     So, target has queued expression. */
  target = protect_from_queue (target, 1);
#endif
     ...
     register tree exp;
{
  ...
  sym = gen_rtx (MEM,
		 ...
		 gen_rtx (PLUS, SImode,
		          sx_greg,
		          temp = gen_rtx (SYMBOL_REF, SImode, "__sx_relocate")));
  reg = gen_reg_rtx (SImode);
  emit_move_insn (reg, sym);
  emit_move_insn (reg, gen_rtx (MEM, SImode, reg));
  mem = gen_rtx (PLUS, SImode, reg, XEXP (DECL_RTL (exp), 0));
  if (memory_address_p (DECL_MODE (exp), mem))
    return gen_rtx (MEM, DECL_MODE (exp), mem);
  else
    return gen_rtx (MEM,
    		    DECL_MODE (exp),
    		    memory_address (DECL_MODE (exp), mem));
}

void
force_pending_stack_adjust ()
{
     ...
     enum expand_modifier modifier;
{
  ...
  switch (code)
    ...
    case PARM_DECL:
      if (DECL_RTL (exp) == 0)
	{
	  error_with_decl (exp, "パラメータのサイズが`%s'に依存しています");
	  return const0_rtx;
	}
	...
	if (GET_CODE (DECL_RTL (exp)) == REG
	    && REGNO (DECL_RTL (exp)) == 8
	    && strcmp (IDENTIFIER_POINTER (DECL_NAME (exp)), "_SXCALLPtr"))
          {
            if (befor_sx_call_val)
              {
              	rtx ret = befor_sx_call_val;
              	befor_sx_call_val = 0;
              	return ret;
              }
	  }
     ...
     int ignore;
{
  ...
  switch (DECL_FUNCTION_CODE (fndecl))
	...
	if (MARIKO_A)
	  {
	    extern flag_interrupt;
	    if (!flag_interrupt && !TREE_INTERRUPT (current_function_decl))
	      warning ("関数に割り込み属性指定がありません");
	    if (arglist)
	      {
		rtx mem;
		rtx tem = expand_expr (TREE_VALUE (arglist), 0, VOIDmode, 0);
		if (GET_CODE (tem) == CONST_INT && INTVAL (tem)==0 )
		  {
		    flag_interrupt = 2;
		    return gen_rtx (CONST_INT, VOIDmode, 0);
		  }
		mem = gen_rtx (MEM,SImode,plus_constant (frame_pointer_rtx, 4));
  ...
  if (GET_MODE (funexp) == EPmode)
      ...
      else
	{
	  ...
	  if (optimize
	      ...
	      && MEM_IN_REMOTE (funexp))
	    {
	      int flag = MEM_IN_PCR (funexp);
	      MEM_IN_PCR (funexp) = 0;
	      funexp = force_reg (Pmode, funexp);
	      MEM_IN_PCR (funexp) = flag;
	    }
#else
     ...
     int ignore;
{
...
#ifdef __human68k__
  if (fndecl && TREE_DOSCALL (fndecl))
    {
      old_target_flags = target_flags;
      /* -mshort same */
      target_flags |= 040;
      /* can't regparm */
      target_flags &= ~020;
  ...
  if (MARIKO_A && (pending_stack_adjust > 0
  		   && fndecl != 0
		   && (!strcmp (IDENTIFIER_POINTER (DECL_NAME (fndecl)), "B_SUPER")
		       || !strcmp (IDENTIFIER_POINTER (DECL_NAME (fndecl)), "_dos_super")
		       || !strcmp (IDENTIFIER_POINTER (DECL_NAME (fndecl)), "_iocs_b_super")
                       || !strcmp (IDENTIFIER_POINTER (DECL_NAME (fndecl)), "SUPER"))))
    do_pending_stack_adjust ();
  ...
  if (fndecl && is_const)
    {
	 ...
	 is deleted, or cse'd.  */
#ifdef __human68k__
      if (! flag_no_function_cse && ! TREE_DOSCALL (fndecl))
#else
...
/* ???  Nothing has been done here to record control flow
  ...
  if (fndecl && TREE_DOSCALL (fndecl))
    {
      target_flags = old_target_flags;
      if (!flag_no_sx)
        {
          rtx tem;
          tem = gen_reg_rtx (SImode);
          befor_sx_call_val = tem;
          emit_move_insn (tem, sx_ret_regs);
	}
    }
#endif
  return target;
