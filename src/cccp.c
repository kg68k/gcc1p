...
#ifndef VMS
#ifdef __human68k__
    { &include_defaults[1], "$gcc_include" },
    { 0, "$include" }
#else
...
#ifdef __human68k__
  max_include_len
    = max (max (strlen(cplusplus_include_defaults[0].fname),
		strlen(cplusplus_include_defaults[1].fname)),
	   strlen(cplusplus_include_defaults[2].fname))+1;
#else
  max_include_len
...
get_filename:
  ...
  if (f < 0) {
    ...
    if (print_deps > (system_header_p || (system_include_depth > 0))) {
      if (system_header_p)
	warning ("存在しないfile <%.*s>は依存関係から外します",
		 fend - fbeg, fbeg);
      else
	{
     ...
     char *msg;
{
  ...
  if (ip != NULL)
    fprintf (gcc_err_file, fmt_warn, ip->fname, ip->lineno);
...
#endif
  if (ip != NULL)
    fprintf (gcc_err_file, fmt_error, ip->fname, line);
  fprintf (gcc_err_file, msg, arg1, arg2, arg3);
  fprintf (gcc_err_file, "\n");
  errors++;
  return 0;
...
void
human68k_setpath(d)
     register struct file_name_list *d;
{
