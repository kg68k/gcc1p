...
gcc handles '-x', '-o outputfile', and '-b base-address' '-as-symbols=N' lk.x's options.
gcc handles options '-cpp-stack=N', '-cc1-stack=N' are available.
...
#define WORD_SWITCH_TAKES_ARG(STR) (*((STR)+5) == 0 && !strncmp (STR, "Tdata", 5))
...
#define Myexit(VAL) \
do { if ((VAL) && X68K_EXIT_BELL) fputc ('\a', stderr);exit ((VAL));} while (0)
#endif	/* __human68k__ */
  ...
  for (i = 0; i < n_commands; i++)
    {
      ...
      int program_is_as =0;
      FILE *fp;
#define LK_SWITCH_TAKES_ARG(c) ((c) == 'o' || (c) == 'm' || (c) == 'b' || (c) == 'i' || (c) == 't')
      ...
      for (cp = compilers; cp->spec; cp++)
	{
	  if (strlen (cp->suffix) < input_filename_length
	      ...
	      && (!strcmp (cp->suffix,
			  ...
			  - strlen (cp->suffix)))
		  || (!strcmp (cp->suffix, ".s")
		      && !strcmp (infiles[i] + input_filename_length - 4
		                  ,".has")))
#else
	      && !strcmp (cp->suffix,
			  infiles[i] + input_filename_length
