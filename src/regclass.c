...
char inter_use_regs[FIRST_PSEUDO_REGISTER];
...
init_reg_sets_1 ()
{
  ...
  for (i = 0; i < FIRST_PSEUDO_REGISTER; i++)
	...
	if (call_used_regs[i] && ! fixed_regs[i])
	  warning ("関数で破壊されるレジスタがglobalレジスタ変数に使われています");
	fixed_regs[i] = 1;
...
{
  int i;
  FRAME_POINTER_REGNUM = REGNO (frame_pointer_rtx) = mariko_forever;
  for (i = 0; i < FIRST_PSEUDO_REGISTER; i++)
	inter_use_regs[i] = 0;
}
     ...
     char **constraints;
{
  ...
  for (p = constraint; *p || next; p++)
	...
	case 'r':
	  ...
	  if (addr_reg_alloc) class = BASE_REG_CLASS;
	  break;
