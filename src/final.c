/* Convert RTL to assembler code and output it, for GNU compiler.
   ...
   Copyright (C) 1991 06 09 FSHARP Mariko Modify
     ...
     char *filename;
{
  int i;
#ifdef __human68k__
  if (profile_block_flag || pragma_profile_block_flag)
#else
  if (profile_block_flag)
#endif
    {
	 ...
	 6:  extra word added by GNU: address of address table
      ...
      ASM_OUTPUT_INTERNAL_LABEL (asm_out_file, "?PBX", 0);
      assemble_integer_zero ();

      ASM_GENERATE_INTERNAL_LABEL (name, "?PBX", 1);
      ASM_OUTPUT_INT (asm_out_file, gen_rtx (SYMBOL_REF, Pmode, name));
      ASM_GENERATE_INTERNAL_LABEL (name, "?PBX", 2);
      ASM_OUTPUT_INT (asm_out_file, gen_rtx (SYMBOL_REF, Pmode, name));
      ASM_OUTPUT_INT (asm_out_file, gen_rtx (CONST_INT, VOIDmode,
					     count_basic_blocks));
      assemble_integer_zero ();
      ASM_GENERATE_INTERNAL_LABEL (name, "?PBX", 3);
      ASM_OUTPUT_INT (asm_out_file, gen_rtx (SYMBOL_REF, Pmode, name));

      /* Output the file name.  */
      ASM_OUTPUT_INTERNAL_LABEL (asm_out_file, "?PBX", 1);
      {
	...
	assemble_string (data_file, strlen (data_file) + 1);
      ...
      ASM_OUTPUT_INTERNAL_LABEL (asm_out_file, "?PBX", 2);
      ASM_OUTPUT_SKIP (asm_out_file, UNITS_PER_WORD * count_basic_blocks);

      /* Output the table of addresses.  */
      text_section ();
      ASM_OUTPUT_INTERNAL_LABEL (asm_out_file, "?PBX", 3);
      for (i = 0; i < count_basic_blocks; i++)
	{
	  char name[12];
	  ASM_GENERATE_INTERNAL_LABEL (name, "?PB", i);
	  ASM_OUTPUT_INT (asm_out_file, gen_rtx (SYMBOL_REF, Pmode, name));
	}
     ...
     FILE *file;
{
  ...
  ASM_OUTPUT_INTERNAL_LABEL (file, "?P", profile_label_no);
  assemble_integer_zero ();
#ifdef __human68k__
  assemble_integer_zero ();
  assemble_integer_zero ();
  ...
  assemble_integer_zero ();
  {
    ...
    if (current_function_decl)
      {
        buf = alloca (strlen (DECL_PRINT_NAME (current_function_decl)) + 10);
        if (!TREE_PUBLIC (current_function_decl))
          strcpy (buf,"static ");
        else
          buf[0] = 0;
        strcat (buf, DECL_PRINT_NAME (current_function_decl));
      }
    else buf = "unkown";
    assemble_string (buf, strlen (buf) + 1);
    fprintf (file, "\t.even\n");
  }
#endif
  text_section ();
...
#ifdef __human68k__
  if (profile_flag)
    FUNCTION_PROFILER_END (file, profile_label_no - 1);
#endif
    ...
    rtx x;
{
  ...
  for (i = GET_RTX_LENGTH (GET_CODE (x)) - 1; i >= 0; i--)
    {
      if (fmt[i] == 'e')
	if (find_mem_double (XEXP (x, i)))
	  return 1;
      if (fmt[i] == 'E')
	{
	  register int j;
	  for (j = 0; j < XVECLEN (x, i); j++)
	    if (find_mem_double (XVECEXP (x, i, j)))
	      return 1;
	}
    }
  return 0;
  ...
  switch (GET_CODE (insn))
	  ...
	  && GET_CODE (NEXT_INSN (insn)) == JUMP_INSN)
	{
	  ...
	  if (GET_CODE (nextbody) == ADDR_VEC
	    ...
	    {
	      ASM_OUTPUT_CASE_LABEL (file, "?", CODE_LABEL_NUMBER (insn),
				     NEXT_INSN (insn));
	      break;
	    }
	}
	...
	if (GET_CODE(body)==SET && GET_CODE(SET_SRC(body))==SYMBOL_REF)
	   if (GET_CODE(SET_DEST(body))!=REG || REGNO(SET_DEST(body)) < 8)
	      bsr_seen = 1;
	if (profile_block_flag && new_block)
	  {
		...
		ASM_OUTPUT_INTERNAL_LABEL (file, "?PB", count_basic_blocks);
	...
	if (optimize
	    ...
	    && !(GET_CODE(SET_SRC (body)) == MEM
	         && memory_indirect_address_p (XEXP (SET_SRC (body), 0))))
	  {
	...
	if (! constrain_operands (insn_code_number))
	  {
	    error ("insn エラー insn番号 %d", insn_code_number);
	    debug_rtx (insn);
	    exit (128);
	  }
#endif
	...
	NOTICE_UPDATE_CC (body, insn);
        ...
        if (GET_CODE (body) == SET)
          {
            if (flag_fpu_hard_bug
                && needs_nop_code
                && ((GET_CODE (SET_SRC (body)) == REG
                     && (int) GET_MODE (SET_SRC (body)) > (int) SImode)
                    || (GET_CODE (SET_DEST (body)) == REG
                        &&  (int) GET_MODE (SET_DEST (body)) > (int) SImode))
                && find_mem_double (body))
              {
                fprintf (file, "\tnop\t\t*fpu delay\n");
                needs_nop_code = 0;
              }
            else
              needs_nop_code = 0;
     ...
     char *str;
{
  if (this_is_asm_operands)
    error_for_asm (this_is_asm_operands, "不正な`asm'です: %s", str);
  else
    abort ();
}

      ...
      else
	{
	  ...
	  /* % followed by a letter and some digits
		   ...
		   || (*p >= 'A' && *p <= 'Z'))
	    {
	      ...
	      if (! (*p >= '0' && *p <= '9'))
		output_operand_lossage ("オペランドの後に%がありません");
	      else if (this_is_asm_operands && c >= (unsigned) insn_noperands)
		output_operand_lossage ("オペランドが範囲を越えました");
	      else if (letter == 'l')
	  ...
	  /* % followed by a digit outputs an operand the default way.  */
		...
		output_operand_lossage ("オぺランドが範囲を越えました");
	      else
	  ...
	  /* % followed by punctuation: output something for that
	    ...
	    output_operand_lossage ("不正な%%-codeです");
	}
    }
     ...
     rtx x;
{
  ...
  if (GET_CODE (x) == LABEL_REF)
    ASM_GENERATE_INTERNAL_LABEL (buf, "?", CODE_LABEL_NUMBER (XEXP (x, 0)));
  else if (GET_CODE (x) == CODE_LABEL)
    ASM_GENERATE_INTERNAL_LABEL (buf, "?", CODE_LABEL_NUMBER (x));
  else
    output_operand_lossage ("`%l'はラベルではありません");
...
{
  char buf[256];
  static char flag,lev;
  extern flag_generic_asm;
  lev++;
 restart:
  switch (GET_CODE (x))
    {
    case SYMBOL_REF:
      ...
      if (MEM_MAY_IN_PCR (x) && !bsr_seen)
        flag = 1;
      if (MEM_IN_PCR (x) && !bsr_seen)
        flag = 2;
      bsr_seen=1;
      break;
	...
	output_operand_lossage ("floating定数が誤使用されています");
      break;

    case PLUS:
      if ((GET_CODE (XEXP (x, 0)) == REG || GET_CODE (XEXP (x, 0)) == PC)
          && GET_CODE (XEXP (x, 1)) == SYMBOL_REF)
        if (REGNO (XEXP (x, 0)) == 13 || GET_CODE (XEXP (x, 0)) == PC)
          output_addr_const (file, XEXP (x, 1));
      	else
      	  abort ();
      /* Some assemblers need integer constants to appear last (eg masm).  */
      else if (GET_CODE (XEXP (x, 0)) == CONST_INT)
	{
    ...
    default:
      output_operand_lossage ("オペランドが不正です");
    }
    lev--;
    if (flag && lev==0)
     {
       if (flag == 2 || flag_all_short)
         if (flag_long_offset)
           fprintf(file,"(pc)");
         else
           fprintf(file,".w(pc)");
