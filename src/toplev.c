...
void print_time(str, total) char *str;
int total;
{
...
static void report_file(line) int line;
{
  extern int cur_lex_pos;
  extern int cur_lex_line;
  if (flag_text_report && cur_lex_line == line && finput != stdin) {
    ...
    fputc('\n', gcc_err_file);
    while (outc-- > 0) fputc(' ', gcc_err_file);
    fputs("^\n", gcc_err_file);
    for (;;) {
      c = fgetc(finput);
      if (c == '\n' || c == EOF)
...
void fatal_io_error(name) char *name;
{
  fprintf(gcc_err_file, "%s: %s: I/O error\n", progname, name);
  exit(35);
}

void fatal(s, v) char *s;
int v;
{
  error(s, v);
  exit(34);
...
void fatal_insn_not_found(insn) rtx insn;
{
  error("コンパイラ内部エラーです", 0);
  debug_rtx(insn);
  insn = get_insns();
  if (insn) print_rtl(stderr, insn);
  abort();
}
...
void announce_function(decl) tree decl;
{
...
void report_error_function(file) char *file;
{
  struct file_stack *p;
  ...
  if (last_error_function != current_function_decl) {
    if (file) fprintf(gcc_err_file, "%s: ", file);
  ...
  if (input_file_stack && input_file_stack->next != 0 &&
    ...
    for (p = input_file_stack->next; p; p = p->next) {
      ...
      if (p->next) fprintf(gcc_err_file, ",");
    }
    fprintf(gcc_err_file, ":\n");
...
static void mariko_err_func() {
  if (MARIKO_D || MARIKO_E) {
    if (!mariko_err_file) {
      ...
      strcat(p, "gcc.err");
      if (fp = fopen(p, "w")) mariko_err_file = fp;
    }
  }
}
...
void error_parse(s, v, v2) char *s;
int v;  /* @@also used as pointer */
int v2; /* @@also used as pointer */
{
  error_with_file_and_line(input_filename, lineno, s, v, v2);
...
void error_with_file_and_line(file, line, s, v, v2) char *file;
int line;
char *s;
int v;
int v2;
{
  ...
  if (file) {
    ...
    if (!mariko_err_file) mariko_err_func();
    if (mariko_err_file) {
      fprintf(mariko_err_file, "%s%s  %5d: Error  : ", file, default_colon,
              line);
...
/* Report an error at the declaration DECL.
   S and V are a string and an arg which uses %s to substitute the declaration
   name.  */

void error_with_decl(decl, s, v) tree decl;
char *s;
int v;
{
  count_error(0);
  ...
  fprintf(gcc_err_file, "%s%s  %5d: Error  : ", DECL_SOURCE_FILE(decl),
          default_colon, DECL_SOURCE_LINE(decl));
  ...
  if (mariko_err_file) {
    fprintf(mariko_err_file, "%s%s  %5d: Error  : ", DECL_SOURCE_FILE(decl),
            default_colon, DECL_SOURCE_LINE(decl));
...
void error_for_asm(insn, s, v, v2) rtx insn;
char *s;
int v;  /* @@also used as pointer */
int v2; /* @@also used as pointer */
{
...
void warning_with_file_and_line(file, line, s, v, v2) char *file;
int line;
char *s;
int v;
int v2;
{
  if (count_error(1) == 0) return;
  report_file(line);
  report_error_function(file);
  ...
  if (file) {
    ...
    if (!mariko_err_file) mariko_err_func();
    if (mariko_err_file) {
      fprintf(mariko_err_file, "%s%s  %5d: Warning: ", file, default_colon,
              line);
      fprintf(mariko_err_file, s, v, v2);
      fprintf(mariko_err_file, "\n");
    }
...
int v1;
{
  if (isatty(fileno(stderr))) {
    fprintf(stderr,
            ...
            "0"
            "%s%s  %5d: ",
            input_filename, default_colon, lineno);
...
void warning_with_decl(decl, s, v) tree decl;
char *s;
int v;
{
  if (count_error(1) == 0) return;
  ...
  fprintf(gcc_err_file, "%s%s  %5d: ", DECL_SOURCE_FILE(decl), default_colon,
  ...
  if (mariko_err_file) {
    fprintf(mariko_err_file, "%s%s  %5d: Warning: ", DECL_SOURCE_FILE(decl),
            default_colon, DECL_SOURCE_LINE(decl));
...
void warning_for_asm(insn, s, v, v2) rtx insn;
char *s;
int v;  /* @@also used as pointer */
int v2; /* @@also used as pointer */
{
...
void really_sorry(s, v, v2) char *s;
int v, v2;
{
  if (input_filename)
    fprintf(gcc_err_file, "%s:%d: ", input_filename, lineno);
...
unsigned size;
{
  ...
  if (value == 0) fatal("メモリがありません");
  return value;
}

/* Same as `realloc' but report error if no memory available.  */

int xrealloc(ptr, size)
char *ptr;
int size;
{
  int result = (int)realloc(ptr, size);
  if (!result) fatal("メモリがありません");
  return result;
}
...
void set_float_handler(handler) jmp_buf handler;
{
  float_handled = (handler != 0);
  if (handler) bcopy(handler, float_handler, sizeof(float_handler));
}

/* Signals actually come here.  */

static void float_signal() {
  if (float_handled == 0) abort();
...
static void compile_file(name) char *name;
{
  tree globals;
  int start_time;
...
#ifdef __human68k__
  if (dump_base_name == 0 || readdump) dump_base_name = name ? name : "gccdump";
#else
  if (dump_base_name == 0) dump_base_name = name ? name : "gccdump";
#endif
...
#endif
  if (finput == 0) pfatal_with_name(name);
  ...
  if (rtl_dump) {
    ...
    if (rtl_dump_file == 0) pfatal_with_name(dumpname);
  }
  ...
  if (jump_opt_dump) {
    ...
    if (jump_opt_dump_file == 0) pfatal_with_name(dumpname);
  }
  ...
  if (cse_dump) {
    ...
    if (cse_dump_file == 0) pfatal_with_name(dumpname);
  }
  ...
  if (loop_dump) {
    ...
    if (loop_dump_file == 0) pfatal_with_name(dumpname);
  }
  ...
  if (flow_dump) {
    ...
    if (flow_dump_file == 0) pfatal_with_name(dumpname);
  }
  ...
  if (combine_dump) {
    ...
    if (combine_dump_file == 0) pfatal_with_name(dumpname);
  }
  ...
  if (local_reg_dump) {
    ...
    if (local_reg_dump_file == 0) pfatal_with_name(dumpname);
  }
  ...
  if (global_reg_dump) {
    ...
    if (global_reg_dump_file == 0) pfatal_with_name(dumpname);
  }
  ...
  if (jump2_opt_dump) {
    ...
    if (jump2_opt_dump_file == 0) pfatal_with_name(dumpname);
  }
  ...
  if (dbr_sched_dump) {
    ...
    if (dbr_sched_dump_file == 0) pfatal_with_name(dumpname);
  }
#endif
  ...
  if (!name_specified && asm_file_name == 0)
    ...
    if (asm_out_file == 0) pfatal_with_name(asm_file_name);
  }
  ...
  if (main_input_filename == 0) main_input_filename = name;
  ...
  if (write_symbols == GDB_DEBUG) {
    ...
    if (len > 2 && !strcmp(".c", dumpname + len - 2))
      ...
      dumpname[len - 3] = 0;
    strcat(dumpname, ".sym");
    if (sym_file_name == 0) sym_file_name = dumpname;
...
#ifdef __human68k__
  if (!readdump) init_final(main_input_filename);
#else
     ...
     and have not been output so far.  */
  {
    ...
    for (decl = globals; decl; decl = TREE_CHAIN(decl)) {
...
#endif
      {
        ...
        if (!TREE_READONLY(decl) || TREE_USED(decl) || TREE_PUBLIC(decl) ||
      ...
      if (TREE_CODE(decl) == FUNCTION_DECL && !TREE_ASM_WRITTEN(decl) &&
          ...
          !TREE_EXTERNAL(decl) && !flag_syntax_only)
        output_inline_function(decl);

      /* Warn about any function declared static but not defined.  */
      if (warn_unused && TREE_CODE(decl) == FUNCTION_DECL &&
          DECL_INITIAL(decl) == 0 && TREE_EXTERNAL(decl) && !TREE_PUBLIC(decl))
...
#ifndef __human68k__
  if (rtl_dump) fclose(rtl_dump_file);

  if (jump_opt_dump) fclose(jump_opt_dump_file);

  if (cse_dump) fclose(cse_dump_file);

  if (loop_dump) fclose(loop_dump_file);

  if (flow_dump) fclose(flow_dump_file);
  ...
  if (local_reg_dump) fclose(local_reg_dump_file);

  if (global_reg_dump) fclose(global_reg_dump_file);

  if (jump2_opt_dump) fclose(jump2_opt_dump_file);

  if (dbr_sched_dump) fclose(dbr_sched_dump_file);
#endif
...
void rest_of_decl_compilation(decl, asmspec, top_level, at_end) tree decl;
char *asmspec;
int top_level;
int at_end;
{
...
#else
    if (write_symbols == SDB_DEBUG && !TREE_EXTERNAL(decl) &&
        !TREE_SYM_WRITTEN(decl)) {
      TIMEVAR(varconst_time, sdbout_symbol(decl, 0));
      TREE_SYM_WRITTEN(decl) = 1;
    }
...
void rest_of_compilation(decl) tree decl;
{
  ...
  if (DECL_SAVED_INSNS(decl) == 0) {
    ...
    if (flag_inline_functions || TREE_INLINE(decl)) {
      TIMEVAR(integration_time, {
        ...
        if (lose != 0 && specd) warning_with_decl(decl, lose);
        if (lose == 0)
  ...
  if (optimize) {
    ...
    if (tem) TIMEVAR(jump_time, jump_optimize(insns, 0, 0));
  }
  ...
  if (obey_regdecls) {
    ...
    TIMEVAR(flow_time, {
      regclass(insns, max_reg_num());
      stupid_life_analysis(insns, max_reg_num(), 0);
    });
#endif
  } else {
...
#ifndef __human68k__
    TIMEVAR(flow_time, flow_analysis(insns, max_reg_num(), flow_dump_file));
#else
    TIMEVAR(flow_time, flow_analysis(insns, max_reg_num(), 0));
#endif
    if (extra_warnings) uninitialized_vars_warning(DECL_INITIAL(decl));
  }
  ...
  if (global_reg_dump)
    TIMEVAR(dump_time, fprintf(global_reg_dump_file, "\n;; Function %s\n\n",
                               IDENTIFIER_POINTER(DECL_NAME(decl))));
#endif
  /* Unless we did stupid register allocation,
  ...
  TIMEVAR(global_alloc_time, {
    if (!obey_regdecls)
      ...
      reload(insns, 0, global_reg_dump ? global_reg_dump_file : 0);
  });
#else
  TIMEVAR(global_alloc_time, {
  ...
  if (optimize && x68k_only) m68k_optimize(insns);
  ...
  if (write_symbols == GDB_DEBUG) {
    TIMEVAR(symout_time, {
      ...
      DECL_BLOCK_SYMTAB_ADDRESS(decl) =
          symout_function(DECL_INITIAL(decl), DECL_ARGUMENTS(decl), 0);
      symout_function_end();
    });
  } else
#endif
...
#ifdef DBX_DEBUGGING_INFO
  if (write_symbols == DBX_DEBUG) TIMEVAR(symout_time, dbxout_function(decl));
#endif
...
void undump_data(name) char *name;
{
  extern int my_edata;
  extern int *_PEND;
  extern int *_DSTA;
  ...
  if (fno < 0) fatal_io_error("dumpファイルがありません\n");

  _dos_read(fno, &check_buf, sizeof(struct INF));

  if (check_buf.PEND == (int *)-1 && check_buf.DSTA == (int *)-1 &&
      check_buf.DEND == (int *)-1 && check_buf.BSTA == (int *)-1)
    reloc = 1;
  else if (check_buf.PEND != _PEND || check_buf.DSTA != _DSTA ||
           check_buf.DEND != _DEND || check_buf.BSTA != _BSTA ||
           check_buf.BEND != _BEND || check_buf.HSTA != _HSTA)
    fatal_io_error("dumpアドレスがずれています\n");

  _dos_read(fno, &size, sizeof(int));
  if ((int)&my_edata + size > (int)_SSTA) fatal_io_error("Stackが重なります\n");

  if (_dos_read(fno, &my_edata, size) != size) abort();
  if (reloc) {
  ...
  for (i = 1; i < argc; i++)
    if (argv[i][0] == '-' && argv[i][1] != 0) {
      ...
      if (str[0] == 'm')
        ...
        for (j = 0; !found && j < sizeof(f_options) / sizeof(f_options[0]);
             j++) {
...
#ifndef __human68k__
        if (write_symbols == NO_DEBUG) write_symbols = GDB_DEBUG;
#endif
...
#endif /* not USG */
  if (arg_error) exit(2);
  if (errorcount) exit(FATAL_EXIT_CODE);
  if (MARIKO_E && warningcount) exit(FATAL_EXIT_CODE);
  if (sorrycount) exit(FATAL_EXIT_CODE);
  if (flag_syntax_only) exit(1);
...
void set_target_switch(name) char *name;
{
  register int j;
